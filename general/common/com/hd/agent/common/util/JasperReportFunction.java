...
public class JasperReportFunction {
	 ...
	 * @date Feb 28, 2017
	 */
	public static BufferedImage getMatrixImage(String text, int width, int height){
		...
		try {
			...
			bufferedImage=com.google.zxing.client.j2se.MatrixToImageWriter.toBufferedImage(writer.encode(text, com.google.zxing.BarcodeFormat.QR_CODE, width, height));

		}catch (Exception ex){
			logger.error("打印模板生成二维码时失败",ex);
		}
		return bufferedImage;
	}
	 ...
	 * @date Feb 28, 2017
	 */
	public static BufferedImage getBarcode128Image(String text,int width,int height){
		BufferedImage bufferedImage=null;
		try {
			...
			com.google.zxing.MultiFormatWriter writer=new com.google.zxing.MultiFormatWriter();
			bufferedImage=com.google.zxing.client.j2se.MatrixToImageWriter.toBufferedImage(writer.encode(text, BarcodeFormat.CODE_128, width, height));

		}catch (Exception ex){
			logger.error("打印模板生成128条形码时失败",ex);
		}
		return bufferedImage;
	}
	/**
	 ...
	 * @date Feb 28, 2017
	 */
	public static BufferedImage getZxingFormatImage(String text,String type,int width,int height){
		BufferedImage bufferedImage=null;
		...
		try {
			...
			com.google.zxing.MultiFormatWriter writer=new com.google.zxing.MultiFormatWriter();
			bufferedImage=com.google.zxing.client.j2se.MatrixToImageWriter.toBufferedImage(writer.encode(text, barcodeFormat, width, height));

		}catch (Exception ex){
			logger.error("打印模板生成条形码(类型："+type+"）时失败",ex);
		}
		return bufferedImage;
	}

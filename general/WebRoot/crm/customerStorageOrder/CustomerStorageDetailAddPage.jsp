...
<div class="easyui-layout" data-options="fit:true">
    <div data-options="region:'center',border:false">
        <form id="crm-form-customerStorageDetailAddPage">
            <table cellpadding="5" cellspacing="2">
                ...
                <tr>
                    <td>数量：</td>
                    <td><input class="easyui-validatebox len150 goodsNum" value="0" name="unitnum" onfocus="this.select();frm_focus('unitnum');" onblur="frm_blur('unitnum');" /></td>
                    <td>辅数量：</td>
                    <td><input name="auxnum" onfocus="this.select();frm_focus('auxnum');" onblur="frm_blur('auxnum');" class="easyui-validatebox goodsNum" value="0" style="width:60px;" readonly="readonly" /><span id="crm-auxunitname-customerStorageDetailAddPage"></span>
                        <input name="overnum" onfocus="this.select();frm_focus('overnum');" onblur="frm_blur('overnum');" class="easyui-validatebox goodsNum" value="0" style="width:60px;" readonly="readonly" /><span id="crm-unitname-customerStorageDetailAddPage"></span>
                    </td>
                </tr>
                <tr>
                    <td>零售单价：</td>
                    ...
                    <td>零售金额：</td>
                    <td><input type="text" class="len150 easyui-validatebox" id="crm-taxamount-customerStorageDetailAddPage" name="taxamount" onfocus="this.select();frm_focus('taxamount');" onblur="frm_blur('taxamount');" validType="intOrFloat" readonly="readonly" /></td>
                </tr>
    ...
    <div data-options="region:'south',border:false">
        <div class="buttonDetailBG" style="height:30px;text-align:right;">
            ...
            <input type="button" name="savegoon" id="crm-savegoon-customerStorageDetailAddPage" value="继续添加"/>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $(function(){
        $("#crm-goodsId-customerStorageDetailAddPage").goodsWidget({
            ...
            param:[{field:'id',op:'notin',value:ids}],
            canBuySale:"2",
        ...
        $("#crm-savegoon-customerStorageDetailAddPage").click(function(){
            addSaveDetail(true);
        });
    ...
    function boxpriceChange(){
        var $this = $("#crm-boxprice-customerStorageDetailAddPage").css({'background':'url(image/loading.gif) right top no-repeat'});
        var price = $("#crm-boxprice-customerStorageDetailAddPage").val();
        var goodsId = $("input[name=goodsid]").val();
        var taxtype = $("input[name=taxtype]").val();
        var unitnum = $("input[name=unitnum]").val();
        var auxnum = $("input[name=auxnum]").val();
    ...
    function costpriceChange(type, id){
        var $this = $(id).css({'background':'url(image/loading.gif) right top no-repeat'});
        var price = $(id).val();
        var goodsId = $("input[name=goodsid]").val();
        var taxtype = $("input[name=taxtype]").val();
    ...
    function costAmountChange(type,id) { //1含税金额2未税金额
        ...
        $("#crm-costprice-customerStorageDetailAddPage").val(parseFloat(price));
        var date = $("input[name='customerStorageOrder.businessdate']").val();
        $.ajax({
            url:"sales/getAuxUnitNumAndPrice.do",
            dataType:'json',

...
<html>
  ...
  <body>
    ...
    if(type == 'handle') {
    	...
    	if(step == '99') {
       		url = 'oa/oaprepay/oaPrepayViewPage.do?id=${param.id }&step=${param.step }&type=handle&billtype=${param.billtype }&processid=${param.processid }';
    	} else {
    		url = 'oa/oaprepay/oaPrepayHandlePage.do?id=${param.id }&step=${param.step }&type=handle&billtype=${param.billtype }&processid=${param.processid }';
    	}
    } else if(type == 'view') {
        ...
        url = 'oa/oaprepay/oaPrepayPrintPage.do?id=${param.id }&type=print&billtype=${param.billtype }&processid=${param.processid }';
        window.location.href = url;
    }
    
    $(function() {
    
    	$('#oa-panel-oaPrepayPage').panel({
			href: url,
			cache: false,
			maximized: true,
			border: false,
    ...
    function oaPrepay_handle_save_form_submit(callBack, args) {
	...
	function workFormSubmit(call, args) {
        ...
        oaPrepay_handle_save_form_submit(call, args);
	}

...
<html>
  ...
  <body>
    ...
    if(type == 'handle') {
    	...
    	if(step == '99') {
    		url = 'oa/oaCustomerPayViewPage.do?id=${param.id }&step=${param.step }&type=view&processid=${param.processid }';
    	} else if(step == '02') {
    	
    		url = 'oa/oaCustomerPayHandlePage2.do?id=${param.id }&step=${param.step }&type=handle&processid=${param.processid }';
    	} else {
    	
    		url = 'oa/oaCustomerPayHandlePage.do?id=${param.id }&step=${param.step }&type=handle&processid=${param.processid }';
    	}
    } else if(type == 'view') {
        ...
        url = 'oa/oaCustomerPayPrintPage.do?id=${param.id }&step=${param.step }&type=view&processid=${param.processid }';
...
//                        });
				...
				$customerpaydetail.datagrid({
					columns: [[
								...
								options:{
									...
									onSelect: function(data) {
										...
										$.ajax({
											type: 'post',
											dataType: 'json',
											url: 'oa/getCustomerBrandSalesAmount.do',
						...
						{field:'executedate', title: '费用执行时间', width: 90,
							editor: {
								type: 'dateText'
							}
						},
				    ...
				    url: 'oa/selectOaCustomerPayDetailList.do?billid=${param.id }',

...
<%--
        // 供应商
        $('#oa-supplierid2-oagoodsViewPage').on('click', function() {
            ...
            var v = $(this).val();
            $(this).attr('value', v);
            $('#oa-supplierid-oagoodsViewPage').attr('value', (v || ':').split(':')[0]);
        });
...
<%--
    ...
    function selectSupplier(data) {
        ...
        $('#oa-supplierid2-oagoodsViewPage').val(data.id + ':' + data.name);
        $('#oa-supplierid-oagoodsViewPage').val(data.id);

        $('#oa-supplierid2-oagoodsViewPage').blur();
        $('#oa-supplierid2-oagoodsViewPage').change();
    }
...
--%>
    ...
    function refreshGoods(e) {
        ...
        $('#oa-goodsdetail-oagoodsViewPage').find(':not(:eq(0))').remove();
        $('#oa-goodsdetail-oagoodsViewPage').append(html.join(''));
        numberPrecision();

        try {

            $('#oa-goodsdetail-oagoodsViewPage').listview('refresh');
        } catch(e) {}
    }
    ...
    */
    function workFormSubmit(call, args) {
        ...
        // 验证
        $('#oa-form-oagoodsViewPage').validate({
            focusInvalid: true,
            debug: true
        });

        $("#oa-form-oagoodsViewPage").submit();
        var flag = $('#oa-form-oagoodsViewPage').validate().form();
        if(!flag) {
        ...
        $('#oa-gooddetail-oagoodsViewPage').val(JSON.stringify(goodsList));

        $('#oa-form-oagoodsViewPage').submit(function(){
...
<div data-role="page" id="main">
    ...
    <form action="" id="oa-form-oagoodsViewPage" method="post">
        <input type="hidden" name="goods.id" id="oa-id-oagoodsViewPage" value="${param.id }"/>
        <input type="hidden" name="goodsBrandJSON" id="oa-gooddetail-oagoodsViewPage"/>
        <div class="ui-corner-all custom-corners">
            <div class="ui-body ui-body-a">
                <div class="ui-field-contain">
                    <label>供应商<font color="#F00">*</font>
                        <textarea name="goods.supplierid2" id="oa-supplierid2-oagoodsViewPage" class="required" readonly="readonly"><c:if test='${not empty supplier }'><c:out value='${goods.supplierid }'/>:<c:out value='${supplier.name }'/></c:if></textarea>
                        <input type="hidden" name="goods.supplierid" id="oa-supplierid-oagoodsViewPage" value="${goods.supplierid }">
                    </label>
                </div>
                <div class="ui-field-contain">
                    <label>进场商家数<font color="#F00">*</font>
                        <input type="number" precision="0" class="required" name="goods.customernum" id="oa-customernum-oagoodsViewPage" value="${goods.customernum }" data-clear-btn="false" maxlength="15" readonly="readonly"/>
                    </label>
                </div>
                <div class="ui-field-contain">
                    <label>进场费用<font color="#F00">*</font>
                        <input type="number" precision="2" class="required" name="goods.costamount" id="oa-costamount-oagoodsViewPage" value="${goods.costamount }" data-clear-btn="false" maxlength="15" readonly="readonly"/>
                    </label>
                </div>
                <div class="ui-field-contain">
                    <label>说明
                        <textarea name="goods.remark" id="oa-remark-oagoodsViewPage" readonly="readonly"><c:out value="${goods.remark }"/></textarea>
                    </label>
                </div>
                <div class="ui-field-contain">
                    <ul data-role="listview" id="oa-goodsdetail-oagoodsViewPage" data-inset="false">
    ...
    <c:if test="${param.type eq 'handle'}">
        <div id="oa-footer-oagoodsViewPage" data-role="footer" data-position="fixed">
            <c:choose>

...
<html>
  ...
  <style type="text/css">
	...
	<div class="easyui-menu" id="oa-contextMenu-oaAccessPage">
		<div id="oa-addRow-oaAccessPage" data-options="iconCls:'button-add'">添加</div>
		<div id="oa-editRow-oaAccessPage" data-options="iconCls:'button-edit'">编辑</div>
		<div id="oa-removeRow-oaAccessPage" data-options="iconCls:'button-delete'">删除</div>
	</div>
    ...
    if(type == 'add') {
    	url = 'oa/oaAccessAddPage.do';
    } else if(type == 'view') {
    	url = 'oa/oaAccessViewPage.do?id=${param.id }&processid=${param.processid }';
    } else if(type == 'handle') {

    	url = 'oa/oaAccessHandlePage4.do?id=${param.id }&oaid=${param.processid }&step=${param.step }&type=handle&processid=${param.processid }&billtype=${param.billtype }';
    	if(step == '01') {
    		url = 'oa/oaAccessHandlePage1.do?id=${param.id }&oaid=${param.processid }&step=${param.step }&type=handle&processid=${param.processid }&billtype=${param.billtype }';
    	} else if(step == '02') {
    		url = 'oa/oaAccessHandlePage2.do?id=${param.id }&oaid=${param.processid }&step=${param.step }&type=handle&processid=${param.processid }&billtype=${param.billtype }';
    	} else if(step == '03') {
    		url = 'oa/oaAccessHandlePage3.do?id=${param.id }&oaid=${param.processid }&step=${param.step }&type=handle&processid=${param.processid }&billtype=${param.billtype }';
    	} else if(step == '04') {
    		url = 'oa/oaAccessHandlePage4.do?id=${param.id }&oaid=${param.processid }&step=${param.step }&type=handle&processid=${param.processid }&billtype=${param.billtype }';
    	} else if(step == '00') {
			url = 'oa/oaAccessHandlePage.do?id=${param.id }&oaid=${param.processid }&step=${param.step }&type=handle&processid=${param.processid }&billtype=${param.billtype }';
		}
    } else if(type == 'print') {
    	url = 'oa/oaAccessPrintPage.do?id=${param.id }&type=print&processid=${param.processid }&billtype=${param.billtype }';
			...
			onLoad: function() {
				...
				$goodsprice.datagrid({
					columns: [[{field:'id', title: '编号', hidden: true},
								...
								{field:'goodsname', title: '商品名称', width: 220, editor: {type: 'validatebox', options:{required: /*true*/false}}},
				    ...
				    url: 'oa/selectOaAccessGoodsPriceList.do?billid=${param.id }',
					...
					<c:if test="${param.step == '01' or param.step eq '00' }">
				...
				$goodsamount.datagrid({
					columns: [[{field:'id', title: '编号', hidden: true},
								  ...
								  {field:'goodsname', title: '商品名称', width: 240},
				    ...
				    url: 'oa/selectOaAccessGoodsAmountList.do?billid=${param.id }',
                    onLoadSuccess: function(data) {
				    ...
				    <c:if test="${param.step == '02' or param.step eq '00'}">
				...
				$branddiscount.datagrid({
				    ...
				    url: 'oa/selectOaAccessBrandDiscountList.do?billid=${param.id }',
					onLoadSuccess: function(data) {
				...
				<c:choose>
					<c:when test="${param.step == '01' or param.step eq '00'}">
					</c:when>
					<c:otherwise>
						// 供应商
				...
				$expensesort.widget({
					...
					col:'expensesort',
					<c:choose>
						<c:when test="${param.step != '01' and param.step ne '00' }">
							readonly: true,
							width: 174,
						</c:when>
						<c:otherwise>
					...
					onChecked: function(data) {
						...
						if(expensesortvalue == null || expensesortvalue == '') {
							...
							$confirmoption.val(data.name);
						
							// $pricetype.children().first().attr('selected', 'selected');
						...
						if(expensesort_text.length > 0 && pricetype_text.length == 0) {

							$confirmoption.val(expensesort_text + pricetype_text);
						} else if(expensesort_text.length == 0 && pricetype_text.length > 0) {
				...
				$pricetype.change(function() {
					...
					//$confirmoption.val($(this).find('option:selected').text());
				...
				// 是否补库存
				$isaddstorage.change(function() {
					$comfirmaddstorage.val($(this).find('option:selected').text());
				});
				...
				$comfirmaddstorage.val($isaddstorage.find('option:selected').text());
				
				if($pricetype.find('option:selected').text() != '' && $expensesort.widget('getText') != '') {
				} else {
				
					$confirmoption.val($pricetype.find('option:selected').text() + $expensesort.widget('getText'));
				}

				/*
				 ** 确认时间、降价电脑设定时间切换
				if($pricetype.find('option:selected').val() == '2') {
					$('.condate').hide();
					$('.comdate').show();
				} else {
					$('.condate').show();
				...
				*/
                ...
                /*
					$comment1.comments({
						...
						processid: '${param.processid }',
						taskkey: 'brandApply,departmentManager,financialManager,generalManager',
						agree: '1',
						width: 185
					});
					...
					$comment2.comments({
						...
						definitionkey: '${param.definitionkey }',
						processid: '${param.processid }',
						taskkey: 'salesManager2,datafileManager',
						agree: '1',
						width: 375
					});
					...
					$comment5.comments({
						...
						definitionkey: '${param.definitionkey }',
						processid: '${param.processid }',
						taskkey: 'salesManager3,accountant',
						agree: '1',
						width: 375
					});
					*/
					...
					$comment3.comments({
						...
						definitionkey: '${param.definitionkey }',
						processid: '${param.processid }',
						taskkey: 'generalManagerAuditDiscount,internalRegister,dataManagerAudit,cfoAudit',
						agree: '1',
						width: 182
					});
					*/
					...
					$comment4.comments({
						...
						processid: '${param.processid }',
						definitionkey: '${param.definitionkey }',
						taskkey: 'generalManagerAuditPay,cashierPay,accountantExamine,financeManagerAudit,cashierReceiveTicket',
						agree: '1',
						width: 145
					});
					*/
				...
				//	$comment1container.panel('close');
				//	$comment2container.panel('close');
				//	$comment3container.panel('close');
				//	$comment4container.panel('close');
				//	$comment5container.panel('close');
				//}
				
				$planbegindate.blur(copyDate);
				$planenddate.blur(copyDate);
				//$pricetype.change(copyDate);
    ...
    function switchInvoiceDiscount() {
		...
		// 支票
			...
			$invoice.panel('open');
			$comment4container.panel('open');

			if(step == '03' || step == '04') {

				$companyname.validatebox({required: true});
				$companyid.validatebox({required: true});
				$bank.validatebox({required: true});
				$bankno.validatebox({required: true});
				$payamount_i.numberbox({required: true});
				$amountwords.validatebox({required: true});
				$expensesort_i.validatebox({required: true});
				$invoicetype.validatebox({required: true});
			}
...
//			$compdiscount.numberbox('clear');
//			$comdownamount.numberbox('clear');
//			$payamount.numberbox('clear');
    ...
    function beginEditPriceRow(rowIndex) {
    	...
    	setTimeout(function(){
	    	...
	    	initGoodsWidget(editors[0].target, goodsid);
			...
			$.ajax({
				...
				success: function(data) {
					...
					$.ajax({
						...
						url: 'oa/computeGoodsByAuxnum.do',
						success: function(data) {
	...
	function initGoodsWidget(d, initid) {
		...
		setTimeout(function() {
			...
			$.ajax({
				type: 'post',
				dataType: 'json',
				url: 'oa/common/selectGoodsInfo.do',
				data: {id: initid},

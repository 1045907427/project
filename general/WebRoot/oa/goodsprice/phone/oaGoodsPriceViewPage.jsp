...
<script type="text/javascript">
    ...
    $(function() {
        ...
        <%--
        // 供应商
        $('#oa-supplierid2-oaGoodsPriceViewPage').on('click', function() {
            ...
            var v = $(this).val();
            $(this).attr('value', v);
            $('#oa-supplierid-oaGoodsPriceViewPage').attr('value', (v || ':').split(':')[0]);
        });
    ...
    function selectSupplier(data) {
        ...
        $('#oa-supplierid2-oaGoodsPriceViewPage').val(data.id + ':' + data.name);
        $('#oa-supplierid-oaGoodsPriceViewPage').val(data.id);

        $('#oa-supplierid2-oaGoodsPriceViewPage').blur();
        $('#oa-supplierid2-oaGoodsPriceViewPage').change();
    }
    ...
    function refreshPrices(e) {
        ...
        $('#oa-pricedetail-oaGoodsPriceViewPage').find(':not(:eq(0))').remove();
        $('#oa-pricedetail-oaGoodsPriceViewPage').append(html.join(''));
        numberPrecision();

        try {

            $('#oa-pricedetail-oaGoodsPriceViewPage').listview('refresh');
        } catch(e) {}
    }
    ...
    */
    function workFormSubmit(call, args) {
        ...
        // 验证
        $('#oa-form-oaGoodsPriceViewPage').validate({
            focusInvalid: true,
            debug: true
        });

        $("#oa-form-oaGoodsPriceViewPage").submit();
        var flag = $('#oa-form-oaGoodsPriceViewPage').validate().form();
        if(!flag) {
        ...
        $('#oa-goodpricedetail-oaGoodsPriceViewPage').val(JSON.stringify(priceList));

        $('#oa-form-oaGoodsPriceViewPage').submit(function(){
...
<div data-role="page" id="main">
    ...
    <form action="" id="oa-form-oaGoodsPriceViewPage" method="post">
        <input type="hidden" name="price.id" id="oa-id-oaGoodsPriceViewPage" value="${param.id }"/>
        <input type="hidden" name="price.oaid" id="oa-oaid-oaGoodsPriceViewPage" value="${param.processid }"/>
        <input type="hidden" name="goodpricedetail" id="oa-goodpricedetail-oaGoodsPriceViewPage"/>
        <div class="ui-corner-all custom-corners">
            <div class="ui-body ui-body-a">
                <div class="ui-field-contain">
                    <label>供应商<font color="#F00">*</font>
                        <input type="text" class="required" name="price.supplierid2" id="oa-supplierid2-oaGoodsPriceViewPage" value="<c:if test='${not empty supplier }'><c:out value='${price.supplierid }'/>:<c:out value='${supplier.name }'/></c:if>" data-clear-btn="false" readonly="readonly"/>
                        <input type="hidden" name="price.supplierid" id="oa-supplierid-oaGoodsPriceViewPage" value="${price.supplierid }">
                    </label>
                </div>
                <div class="ui-field-contain">
                    <label>说明
                        <textarea name="price.remark" id="oa-remark-oaGoodsPriceViewPage" readonly="readonly"><c:out value="${price.remark }"/></textarea>
                    </label>
                </div>
                <div class="ui-field-contain">
                    <ul data-role="listview" id="oa-pricedetail-oaGoodsPriceViewPage" data-inset="false">
    ...
    <c:if test="${param.type eq 'handle'}">
        <div id="oa-footer-oaGoodsPriceViewPage" data-role="footer" data-position="fixed">
            <c:choose>

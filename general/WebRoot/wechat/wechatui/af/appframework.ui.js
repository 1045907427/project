...
(function($) {
    ...
    AFUi.prototype = {
        ...
        goBack: function(e) {
            ...
            if(hist.length>0){
                var toTarget;
                if(document.location.hash == "#" + this.activeDiv.id || document.location.hash == ""){
                    toTarget=hist[hist.length-1].target;
                }else{
                    for(var i = hist.length  ; i > 0 ; i--){
                        if("#" + hist[i - 1].target.id == document.location.hash){
                            toTarget = hist[i - 1].target;
                            break;
                        }else{
                            hist.pop();
                        }
         ...
         * @api private
         */
        loadAjax: function(target, newTab, back, transition, anchor, title) {
            var that=this;
        ...
        launch: function() {
            ...
            if(this.loadDefaultHash){
                ...
                setTimeout(function(){
                    this.loadContent(firsthash,false,0,"slide");
                    //zzh
                    $(this.defaultPanel).trigger("panelbeforeload");
                }.bind(this));
            }
            this.enableTabBar();
            ...
            $(document).undelegate(".backButton, [data-back]","singleTap").delegate(".backButton, [data-back]","singleTap",function(){
                $.afui.goBack();
            });
            //Check for includes
...
(function ($) {
    ...
    popup.prototype = {
        ...
        positionPopup: function () {
            ...
            popup.css("top", ((window.innerHeight / 2.5) + window.pageYOffset) - ($(popup[0]).height() / 2) + "px");
            popup.css("left", (window.innerWidth / 2) - ($(popup[0]).width() / 2) + "px");
        }
    };
    var uiBlocked = false;
    $.blockUI = function (opacity) {

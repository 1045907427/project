...
<html>
  ...
  <body>
    	<table id="report-datagrid-fundsBranduserBrandCustomerReturn"></table>
    	<div id="report-toolbar-fundsBranduserBrandCustomerReturn">
    		<form action="" id="report-query-form-fundsBranduserBrandCustomerReturn" method="post">
    		<table>
    			<tr>
    				<td>业务日期:</td>
    				...
    				<td>品牌业务员:</td>
    				<td><input type="text" id="report-query-branduserid" name="branduser"/></td>
    				<td>商品品牌:</td>
    			...
    			<tr>
    				...
    				<td colspan="2">
    					<a href="javaScript:void(0);" id="report-queay-fundsBranduserBrandCustomerReturn" class="easyui-linkbutton" iconCls="icon-search" plain="true" title="[Alt+Q]查询">查询</a>
						<a href="javaScript:void(0);" id="report-reload-fundsBranduserBrandCustomerReturn" class="easyui-linkbutton" iconCls="icon-reload" plain="true" title="[Alt+R]重置">重置</a>
    					<security:authorize url="/report/finance/fundsBranduserBrandCustomerReturnExport.do">
							<a href="javaScript:void(0);" id="report-buttons-fundsBranduserBrandCustomerReturnPage" class="easyui-linkbutton" iconCls="button-export" plain="true" title="导出">导出</a>
						</security:authorize>
    				</td>
    			</tr>
    		</table>
    	...
    	<script type="text/javascript">
    		var initQueryJSON = $("#report-query-form-fundsBranduserBrandCustomerReturn").serializeJSON();
        	var SR_footerobject = null;
    		$(function(){
    			$("#report-datagrid-fundsBranduserBrandCustomerReturn").datagrid({ 
					columns:[[
			 		...
			 		selectOnCheck:true,
		  	 		pageSize:100,
					toolbar:'#report-toolbar-fundsBranduserBrandCustomerReturn',
					onDblClickRow:function(rowIndex, rowData){
						...
						$("#report-dialog-fundsBranduserBrandCustomerReturnDetail").dialog({
						...
						$("#report-dialog-fundsBranduserBrandCustomerReturnDetail").dialog("open");
					},
				...
				$("#report-query-branduserid").widget({
				...
				//回车事件
				controlQueryAndResetByKey("report-queay-fundsBranduserBrandCustomerReturn","report-reload-fundsBranduserBrandCustomerReturn");
				
				//查询
				$("#report-queay-fundsBranduserBrandCustomerReturn").click(function(){
					//把form表单的name序列化成JSON对象
		      		var queryJSON = $("#report-query-form-fundsBranduserBrandCustomerReturn").serializeJSON();
		      		$("#report-datagrid-fundsBranduserBrandCustomerReturn").datagrid({
		      			url: 'report/finance/showBaseSalesWithdrawnData.do?groupcols=branduser,brandid,customerid',
		      			pageNumber:1,
						queryParams:queryJSON
		      		});
				});
				//重置
				$("#report-reload-fundsBranduserBrandCustomerReturn").click(function(){
					$("#report-query-branduserid").widget("clear");
					$("#report-query-brand").widget("clear");
					$("#report-query-customerid").customerWidget("clear");
					$("#report-query-form-fundsBranduserBrandCustomerReturn").form("reset");
					var queryJSON = $("#report-query-form-fundsBranduserBrandCustomerReturn").serializeJSON();
		       		$("#report-datagrid-fundsBranduserBrandCustomerReturn").datagrid('loadData',{total:0,rows:[]});
				});
				
				$("#report-buttons-fundsBranduserBrandCustomerReturnPage").Excel('export',{
					queryForm: "#report-query-form-fundsBranduserBrandCustomerReturn", //查询条件的form表单，导出时只用通过查询的条件，将通用查询放在form表单里面。
			 		type:'exportUserdefined',
			 		name:'分品牌业务员资金回笼表',
			 		url:'report/finance/exportBaseSalesWithdrawnData.do?groupcols=branduser;brandid;customerid'
				});
    		...
    		function countTotalAmount(){
    			var rows =  $("#report-datagrid-fundsBranduserBrandCustomerReturn").datagrid('getChecked');
        		if(null==rows || rows.length==0){
            		var foot=[];
	    			if(null!=SR_footerobject){
		        		foot.push(SR_footerobject);
		    		}
	    			$("#report-datagrid-fundsBranduserBrandCustomerReturn").datagrid("reloadFooter",foot);
            		return false;
        		}
        		var sendamount = 0;

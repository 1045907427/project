...
<html>
  ...
  <body>
    	<table id="report-datagrid-fundsDepartmentReturn"></table>
    	<div id="report-toolbar-fundsDepartmentReturn">
    		<form action="" id="report-query-form-fundsDepartmentReturn" method="post">
    		<table>
    			<tr>
    				<td>业务日期:</td>
    				...
    				<td colspan="2">
    					<a href="javaScript:void(0);" id="report-queay-fundsDepartmentReturn" class="easyui-linkbutton" iconCls="icon-search" plain="true" title="[Alt+Q]查询">查询</a>
						<a href="javaScript:void(0);" id="report-reload-fundsDepartmentReturn" class="easyui-linkbutton" iconCls="icon-reload" plain="true" title="[Alt+R]重置">重置</a>
    					<security:authorize url="/report/finance/fundsDeptReturnExport.do">
							<a href="javaScript:void(0);" id="report-buttons-fundsDeptReturnPage" class="easyui-linkbutton" iconCls="button-export" plain="true" title="导出">导出</a>
						</security:authorize>
    				</td>
    			</tr>
    		</table>
    	...
    	<script type="text/javascript">
    		var initQueryJSON = $("#report-query-form-fundsDepartmentReturn").serializeJSON();
			var SR_footerobject = null;
    		$(function(){
    			$("#report-datagrid-fundsDepartmentReturn").datagrid({ 
					columns:[[
					          {field:'idok',checkbox:true,isShow:true},
								  ...
								  {field:'allunwithdrawnamount',title:'应收款总额',align:'right',resizable:true,isShow:true,
								  	formatter:function(value,rowData,rowIndex){
								  		if(value!=0){
						        			return '<a href="javascript:showDetail(\''+rowData.salesdept+'\',0);" style="text-decoration:none">'+formatterMoney(value)+'</a>';
						        		}else{
						        			return formatterMoney(value);
						        		}
						        	}
								  ...
								  {field:'auditamount',title:'已验收应收款',align:'right',resizable:true,isShow:true,sortable:true,
								  	formatter:function(value,rowData,rowIndex){
								  		if(value!=0){
						        			return '<a href="javascript:showDetail(\''+rowData.salesdept+'\',1);" style="text-decoration:none">'+formatterMoney(value)+'</a>';
						        		}else{
						        			return formatterMoney(value);
						        		}
						        	}
								  ...
								  {field:'rejectamount',title:'退货应收款',align:'right',resizable:true,isShow:true,sortable:true,
								  	formatter:function(value,rowData,rowIndex){
								  		if(value!=0){
						        			return '<a href="javascript:showDetail(\''+rowData.salesdept+'\',2);" style="text-decoration:none">'+formatterMoney(value)+'</a>';
						        		}else{
						        			return formatterMoney(value);
						        		}
						        	}
			 		...
			 		checkOnSelect:true,
			 		selectOnCheck:true,
					toolbar:'#report-toolbar-fundsDepartmentReturn',
					onDblClickRow:function(rowIndex, rowData){
    					...
    					var url = 'report/finance/showFundsSalesDeptReturnDetailPage.do?salesdept='+salesdept+'&businessdate1='+businessdate1+'&businessdate2='+businessdate2+'&salesdeptname='+salesdeptname;
				...
				//回车事件
				controlQueryAndResetByKey("report-queay-fundsDepartmentReturn","report-reload-fundsDepartmentReturn");
				
				//查询
				$("#report-queay-fundsDepartmentReturn").click(function(){
					//把form表单的name序列化成JSON对象
		      		var queryJSON = $("#report-query-form-fundsDepartmentReturn").serializeJSON();
		      		$("#report-datagrid-fundsDepartmentReturn").datagrid({
		      			url: 'report/finance/showBaseSalesWithdrawnData.do?groupcols=salesdept',
		      			pageNumber:1,
						queryParams:queryJSON
		      		});
				});
				//重置
				$("#report-reload-fundsDepartmentReturn").click(function(){
					$("#report-query-deptid").widget("clear");
					$("#report-query-form-fundsDepartmentReturn")[0].reset();
					var queryJSON = $("#report-query-form-fundsDepartmentReturn").serializeJSON();
		       		$("#report-datagrid-fundsDepartmentReturn").datagrid('loadData',{total:0,rows:[]});
				});
				
				$("#report-buttons-fundsDeptReturnPage").Excel('export',{
					queryForm: "#report-query-form-fundsDepartmentReturn", //查询条件的form表单，导出时只用通过查询的条件，将通用查询放在form表单里面。
			 		type:'exportUserdefined',
			 		name:'分部门资金回笼表',
			 		url:'report/finance/exportBaseSalesWithdrawnData.do?groupcols=salesdept'
				});
    		});
    		...
    		function countTotalAmount(){
    			var rows =  $("#report-datagrid-fundsDepartmentReturn").datagrid('getChecked');
        		if(null==rows || rows.length==0){
            		var foot=[];
	    			if(null!=SR_footerobject){
		        		foot.push(SR_footerobject);
		    		}
	    			$("#report-datagrid-fundsDepartmentReturn").datagrid("reloadFooter",foot);
            		return false;
        		}
        		var sendamount = 0;
        		...
        		var foot=[{salesdeptname:'选中合计',sendamount:sendamount,directreturnamount:directreturnamount,

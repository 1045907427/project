...
<html>
  <head>
    <title>分品牌业务员分品牌资金回笼表</title>
    <%@include file="/include.jsp" %>   
    <script type="text/javascript" src="js/reportDatagrid.js" charset="UTF-8"></script>  
  </head>
  
  <body>
    	<table id="report-datagrid-fundsBranduserBrandReturn"></table>
    	<div id="report-toolbar-fundsBranduserBrandReturn">
    		<form action="" id="report-query-form-fundsBranduserBrandReturn" method="post">
    		<table>
    			<tr>
    				<td>业务日期:</td>
    				...
    				<td colspan="2">
    					<a href="javaScript:void(0);" id="report-queay-fundsBranduserBrandReturn" class="easyui-linkbutton" iconCls="icon-search" plain="true" title="[Alt+Q]查询">查询</a>
						<a href="javaScript:void(0);" id="report-reload-fundsBranduserBrandReturn" class="easyui-linkbutton" iconCls="icon-reload" plain="true" title="[Alt+R]重置">重置</a>
    					<security:authorize url="/report/finance/fundsBranduserBrandReturnExport.do">
							<a href="javaScript:void(0);" id="report-buttons-fundsBranduserBrandReturnPage" class="easyui-linkbutton" iconCls="button-export" plain="true" title="导出">导出</a>
						</security:authorize>
    				</td>
    			</tr>
    		</table>
    	...
    	<script type="text/javascript">
    		var initQueryJSON = $("#report-query-form-fundsBranduserBrandReturn").serializeJSON();
        	var SR_footerobject = null;
    		$(function(){
    			$("#report-datagrid-fundsBranduserBrandReturn").datagrid({ 
					columns:[[
			 		...
			 		checkOnSelect:true,
			 		selectOnCheck:true,
					toolbar:'#report-toolbar-fundsBranduserBrandReturn',
				...
				//回车事件
				controlQueryAndResetByKey("report-queay-fundsBranduserBrandReturn","report-reload-fundsBranduserBrandReturn");
				
				//查询
				$("#report-queay-fundsBranduserBrandReturn").click(function(){
					//把form表单的name序列化成JSON对象
		      		var queryJSON = $("#report-query-form-fundsBranduserBrandReturn").serializeJSON();
		      		$("#report-datagrid-fundsBranduserBrandReturn").datagrid({
		      			url: 'report/finance/showBaseSalesWithdrawnData.do?groupcols=branduser,brandid',
		      			pageNumber:1,
						queryParams:queryJSON
		      		});
				});
				//重置
				$("#report-reload-fundsBranduserBrandReturn").click(function(){
					$("#report-query-branduserid").widget("clear");
					$("#report-query-form-fundsBranduserBrandReturn").form("reset");
					var queryJSON = $("#report-query-form-fundsBranduserBrandReturn").serializeJSON();
		       		$("#report-datagrid-fundsBranduserBrandReturn").datagrid('loadData',{total:0,rows:[]});
				});
				
				$("#report-buttons-fundsBranduserBrandReturnPage").Excel('export',{
					queryForm: "#report-query-form-fundsBranduserBrandReturn", //查询条件的form表单，导出时只用通过查询的条件，将通用查询放在form表单里面。
			 		type:'exportUserdefined',
			 		name:'分品牌业务员资金回笼表',
			 		url:'report/finance/exportBaseSalesWithdrawnData.do?groupcols=branduser,brandid'
				});
    		...
    		function countTotalAmount(){
    			var rows =  $("#report-datagrid-fundsBranduserBrandReturn").datagrid('getChecked');
        		if(null==rows || rows.length==0){
            		var foot=[];
	    			if(null!=SR_footerobject){
		        		foot.push(SR_footerobject);
		    		}
	    			$("#report-datagrid-fundsBranduserBrandReturn").datagrid("reloadFooter",foot);
            		return false;
        		}
        		var sendamount = 0;

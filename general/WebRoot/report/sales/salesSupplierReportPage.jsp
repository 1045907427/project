...
<html>
  <head>
    <title>按供应商销售情况统计报表</title>
    <%@include file="/include.jsp" %> 
    <script type="text/javascript" src="js/reportDatagrid.js" charset="UTF-8"></script>    
  </head>
  
  <body>
    	<table id="report-datagrid-supplierid"></table>
    	<div id="report-toolbar-supplierid" style="padding: 0px">
            <div class="buttonBG">
                <security:authorize url="/report/sales/supplierReportExport.do">
                    <a href="javaScript:void(0);" id="report-autoExport-supplieridPage" class="easyui-linkbutton" iconCls="button-export" plain="true" title="全局导出">全局导出</a>
                </security:authorize>
            </div>
    		<form action="" id="report-query-form-supplierid" method="post">
    		<table class="querytable">
    			...
    			<tr>
    				...
    				<td colspan="2">
    					<a href="javaScript:void(0);" id="report-queay-supplierid" class="button-qr">查询</a>
						<a href="javaScript:void(0);" id="report-reload-supplierid" class="button-qr">重置</a>
    				</td>
    			</tr>
    		</table>
    	</form>
    	</div>
    	<div id="report-dialog-supplierDetail"></div>
    	<script type="text/javascript">
			var SR_footerobject  = null;
    		var initQueryJSON = $("#report-query-form-supplierid").serializeJSON();
    		$(function(){
                $("#report-autoExport-supplieridPage").click(function(){
                    var queryJSON = $("#report-query-form-supplierid").serializeJSON();
                    //获取排序规则
                    var objecr  = $("#report-datagrid-supplierid").datagrid("options");
                    if(null != objecr.sortName && null != objecr.sortOrder ){
                        queryJSON["sort"] = objecr.sortName;
                        queryJSON["order"] = objecr.sortOrder;
                    }
                    var queryParam = JSON.stringify(queryJSON);
                    var url = "report/sales/exportBaseSalesReportData.do?groupcols=supplierid";
                    exportByAnalyse(queryParam,"分供应商销售情况统计报表","report-datagrid-supplierid",url);
                });
                
    			var tableColumnListJson = $("#report-datagrid-supplierid").createGridColumnLoad({
					frozenCol : [[
    			...
    			$("#report-datagrid-supplierid").datagrid({ 
					...
					columns:tableColumnListJson.common,
			 		...
			 		selectOnCheck:true,
		  	 		pageSize:100,
					toolbar:'#report-toolbar-supplierid',
					onDblClickRow:function(rowIndex, rowData){
    					...
    					var url = 'report/sales/showSalesSupplierDetailPage.do?supplierid='+supplierid+'&businessdate1='+businessdate1+'&businessdate2='+businessdate2+'&suppliername='+suppliername;
						$("#report-dialog-supplierDetail").dialog({
						...
						$("#report-dialog-supplierDetail").dialog("open");
					},
		  	 		onLoadSuccess:function(){
						var footerrows = $(this).datagrid('getFooterRows');
						if(null!=footerrows && footerrows.length>0){
							SR_footerobject = footerrows[0];
							baseSalesReportCountTotalAmount('suppliername',this);
						}
			 		},
					onCheckAll:function(){
						baseSalesReportCountTotalAmount('suppliername',this);
				...
				//回车事件
				controlQueryAndResetByKey("report-queay-supplierid","report-reload-supplierid");
				
				//查询
				$("#report-queay-supplierid").click(function(){
					//把form表单的name序列化成JSON对象
		      		var queryJSON = $("#report-query-form-supplierid").serializeJSON();
		      		$("#report-datagrid-supplierid").datagrid({
		      			url: 'report/sales/showBaseSalesReportData.do?groupcols=supplierid',
		      			pageNumber:1,
						queryParams:queryJSON
		      		});
				});
				//重置
				$("#report-reload-supplierid").click(function(){
					$("#report-query-supplierid").widget("clear");
					$("#report-query-form-supplierid")[0].reset();
					var queryJSON = $("#report-query-form-supplierid").serializeJSON();
		       		$("#report-datagrid-supplierid").datagrid('loadData',{total:0,rows:[]});
				});

...
<html>
  ...
  <body>
    	<table id="report-datagrid-salesUser"></table>
    	<div id="report-toolbar-salesUser" style="padding: 0px">
            <div class="buttonBG">
                <security:authorize url="/report/sales/salesUserReportExport.do">
                    <a href="javaScript:void(0);" id="report-autoExport-salesUserPage" class="easyui-linkbutton" iconCls="button-export" plain="true" title="全局导出">全局导出</a>
                </security:authorize>
            </div>
    		<form action="" id="report-query-form-salesUser" method="post">
	    		<table class="querytable">
	    			...
	    			<tr>
	    				...
	    				<td colspan="2">
	    					<a href="javaScript:void(0);" id="report-queay-salesUser" class="button-qr">查询</a>
							<a href="javaScript:void(0);" id="report-reload-salesUser" class="button-qr">重置</a>
    	...
    	<div id="report-dialog-salesUserDetail"></div>
    	<script type="text/javascript">
			var SR_footerobject  = null;
    		var initQueryJSON = $("#report-query-form-salesUser").serializeJSON();
    		$(function(){
                $("#report-autoExport-salesUserPage").click(function(){
                    var queryJSON = $("#report-query-form-salesUser").serializeJSON();
                    //获取排序规则
                    var objecr  = $("#report-datagrid-salesUser").datagrid("options");
                    if(null != objecr.sortName && null != objecr.sortOrder ){
                        queryJSON["sort"] = objecr.sortName;
                        queryJSON["order"] = objecr.sortOrder;
                    }
                    var queryParam = JSON.stringify(queryJSON);
                    var url = "report/sales/exportBaseSalesReportData.do?groupcols=salesuser";
                    exportByAnalyse(queryParam,"分客户业务员销售情况统计报表","report-datagrid-salesUser",url);

                });
                
                var tableColumnListJson = $("#report-datagrid-salesUser").createGridColumnLoad({
					name :'t_report_sales_salesuser',
					frozenCol : [[
				...
				$("#report-datagrid-salesUser").datagrid({ 
					...
					columns:tableColumnListJson.common,
			 		...
			 		selectOnCheck:true,
		  	 		pageSize:100,
					toolbar:'#report-toolbar-salesUser',
					onDblClickRow:function(rowIndex, rowData){
    					...
    					var url = 'report/sales/showSalesUserDetailPage.do?salesuser='+salesuser+'&businessdate1='+businessdate1+'&businessdate2='+businessdate2+'&salesusername='+salesusername;
						$("#report-dialog-salesUserDetail").dialog({
						...
						$("#report-dialog-salesUserDetail").dialog("open");
					},
		  	 		onLoadSuccess:function(){
						var footerrows = $(this).datagrid('getFooterRows');
						if(null!=footerrows && footerrows.length>0){
							SR_footerobject = footerrows[0];
							baseSalesReportCountTotalAmount('salesusername',this);
						}
			 		},
				...
				//回车事件
				controlQueryAndResetByKey("report-queay-salesUser","report-reload-salesUser");
				
				//查询
				$("#report-queay-salesUser").click(function(){
		      		var queryJSON = $("#report-query-form-salesUser").serializeJSON();
		      		$("#report-datagrid-salesUser").datagrid({
		      			url: 'report/sales/showBaseSalesReportData.do?groupcols=salesuser',
		      			pageNumber:1,
						queryParams:queryJSON
		      		});
				});
				//重置
				$("#report-reload-salesUser").click(function(){
					$("#report-query-salesuser").widget("clear");
					$("#report-query-form-salesUser")[0].reset();
					var queryJSON = $("#report-query-form-salesUser").serializeJSON();
		       		$("#report-datagrid-salesUser").datagrid('loadData',{total:0,rows:[]});
				});

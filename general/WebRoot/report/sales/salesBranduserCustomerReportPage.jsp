...
<html>
  ...
  <body>
    	<table id="report-datagrid-salesBrandUserCustomer"></table>
    	<div id="report-toolbar-salesBrandUserCustomer" style="padding: 0px">
            <div class="buttonBG">
                <security:authorize url="/report/sales/salesBrandUserCustomerReportExport.do">
                    <a href="javaScript:void(0);" id="report-autoExport-salesBrandUserCustomerPage" class="easyui-linkbutton" iconCls="button-export" plain="true" title="全局导出">全局导出</a>
                </security:authorize>
            </div>
    		<form action="" id="report-query-form-salesBrandUserCustomer" method="post">
	    		<table class="querytable">
	    			...
	    			<tr>
	    				...
	    				<td colspan="2"></td>
	    				<td colspan="2">
	    					<a href="javaScript:void(0);" id="report-queay-salesBrandUserCustomer" class="button-qr">查询</a>
							<a href="javaScript:void(0);" id="report-reload-salesBrandUserCustomer" class="button-qr">重置</a>
	    				</td>
	    			</tr>
	    		</table>
	    	</form>
    	...
    	<script type="text/javascript">
    		...
    		var initQueryJSON = $("#report-query-form-salesBrandUserCustomer").serializeJSON();
    		$(function(){
                $("#report-autoExport-salesBrandUserCustomerPage").click(function(){
                    var queryJSON = $("#report-query-form-salesBrandUserCustomer").serializeJSON();
                    //获取排序规则
                    var objecr  = $("#report-datagrid-salesBrandUserCustomer").datagrid("options");
                    if(null != objecr.sortName && null != objecr.sortOrder ){
                        queryJSON["sort"] = objecr.sortName;
                        queryJSON["order"] = objecr.sortOrder;
                    }
                    var queryParam = JSON.stringify(queryJSON);
                    var url = "report/sales/exportBaseSalesReportData.do?groupcols=branduser,customerid";
                    exportByAnalyse(queryParam,"分品牌业务员分品牌销售情况统计报表","report-datagrid-salesBrandUserCustomer",url);

                });
                
    			var tableColumnListCustomerJson = $("#report-datagrid-salesBrandUserCustomer").createGridColumnLoad({
					frozenCol : [[
    			...
    			$("#report-datagrid-salesBrandUserCustomer").datagrid({ 
					authority:tableColumnListCustomerJson,
			 		frozenColumns: tableColumnListCustomerJson.frozen,
					columns:tableColumnListCustomerJson.common,
			 		method:'post',
		  	 		fit:true,
		  	 		rownumbers:true,
		  	 		pagination:true,
			 		...
			 		checkOnSelect:true,
			 		selectOnCheck:true,
					toolbar:'#report-toolbar-salesBrandUserCustomer',
				...
				//回车事件
				controlQueryAndResetByKey("report-queay-salesBrandUserCustomer","report-reload-salesBrandUserCustomer");
				
				//查询
				$("#report-queay-salesBrandUserCustomer").click(function(){
		      		var queryJSON = $("#report-query-form-salesBrandUserCustomer").serializeJSON();
		      		$("#report-datagrid-salesBrandUserCustomer").datagrid({
		      			url: 'report/sales/showBaseSalesReportData.do?groupcols=branduser,customerid',
		      			pageNumber:1,
						queryParams:queryJSON
		      		});
				});
				//重置
				$("#report-reload-salesBrandUserCustomer").click(function(){
					$("#report-query-branduser").widget("clear");
					$("#report-query-customer").customerWidget("clear");
					$("#report-query-form-salesBrandUserCustomer")[0].reset();
					var queryJSON = $("#report-query-form-salesBrandUserCustomer").serializeJSON();
					$("#report-datagrid-salesBrandUserCustomer").datagrid('loadData',{total:0,rows:[]});
				});

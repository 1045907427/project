...
<html>
	...
	<body>
    	...
    	<div data-options="region:'north',border:false">
    		<div class="buttonBG" id="storage-buttons-logisticsFollowReportListPage" style="height:26px;"></div>
    	</div>
    	 -->
			<div data-options="region:'center'">
				<table id="report-datagrid-logisticsFollowReportListPage"
					data-options="border:false"></table>
			</div>
		</div>
		<div id="report-datagrid-toolbar-logisticsFollowReportListPage" style="padding: 0px">
            <div class="buttonBG">
                <security:authorize url="/report/logistics/logisticsFollowExport.do">
                    <a href="javaScript:void(0);" id="report-buttons-export" class="easyui-linkbutton" iconCls="button-export" plain="true" title="导出">导出</a>
                </security:authorize>
            </div>
			<form action="" id="report-form-query-logisticsFollowReportListPage" method="post">
		...
		<script type="text/javascript">
     	var initQueryJSON = $("#report-form-query-logisticsFollowReportListPage").serializeJSON();
    	...
    	$(function(){
			...
			var deliveryJson = $("#report-datagrid-logisticsFollowReportListPage").createGridColumnLoad({
				frozenCol : [[
			...
			$("#report-datagrid-logisticsFollowReportListPage").datagrid({ 
				...
				toolbar:'#report-datagrid-toolbar-logisticsFollowReportListPage',
			...
			$("#report-query-advanced-logistics").advancedQuery({
				//查询针对的表
		 		...
		 		//查询针对的表格id
		 		datagrid:'report-datagrid-logisticsFollowReportListPage',
		 		plain:true
			});
			...
			$("#report-queay-logistics").click(function(){
				...
				//把form表单的name序列化成JSON对象
	       		var queryJSON = $("#report-form-query-logisticsFollowReportListPage").serializeJSON();
	       		...
	       		//$("#report-datagrid-logisticsFollowReportListPage").datagrid("load",queryJSON);
			});
			//重置
			$("#report-reload-logistics").click(function(){
				$("#report-userid-logisticsSourceQueryPage").widget("clear");
				$("#report-form-query-logisticsFollowReportListPage")[0].reset();
				//var queryJSON = $("#report-form-query-logisticsFollowReportListPage").serializeJSON();
	       		$("#report-datagrid-logisticsFollowReportListPage").datagrid("loadData",{total:0,rows:[]});
			});

			$("#report-buttons-export").Excel('export',{
					queryForm: "#report-form-query-logisticsFollowReportListPage", //查询条件的form表单，导出时只用通过查询的条件，将通用查询放在form表单里面。

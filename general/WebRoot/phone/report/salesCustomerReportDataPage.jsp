...
<div data-role="page" id="pagetwo">
    <form method="post"  class="form-horizontal" role="form" id="report_form_salesCustomerReportDataPage" data-ajax="false"  action="phone/showBaseFinanceDrawnDataPage.do">
        ...
        <input type="hidden" id="pcustomerid" name="pcustomerid" value="${pcustomerid}"/>
        <input type="hidden" id="salesarea" name="salesarea" value="${salesarea}"/>
        <input type="hidden" id="salesdept" name="salesdept" value="${salesdept}"/>
        <%--<input type="hidden" id="businessdate2" name="businessdate2" value="${businessdate2}"/>--%>
    ...
    <div data-role="header" data-position="fixed">
        ...
        <div data-role="popup" id="myPopup" style="width: 100% ;height:90%;display: none;">
            <div>
                <ul data-role="listview" data-inset="true" id="report_footer_salesCustomerReportPage">
                </ul>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    function searchData() {
        ...
        $('#thelist').html('');
        $("#report_footer_salesCustomerReportPage").html('');//显示合计
        i=0;
        listnumber=0;
        nextpagenumber=0;
    ...
    var initQueryJSON = $("#report-query-form-salesCustomerReportDataPage").serializeJSON();
    $(function(){
        searchData();
    });
...
//        }
    ...
    function Ajax(page){ // ajax后台交互
        ...
        $.ajax({
            ...
            data : $("#report_form_salesCustomerReportDataPage").serialize(),
            success : function(res){
                if(res.list.length){ // 如果后台传过来有数据执行如下操作 ， 没有就执行else 告诉用户没有更多内容呢
                    ...
                    for (var i in res.list) {
                        ...
                        arr.push("</div>");
                        arr.push("<li data-icon='false'  id='main-li'onclick='showDataByCustomer(&apos;"+res.list[i].customerid+"&apos;,&apos;"+res.list[i].customername+"&apos;)'> ");
                        arr.push("<div id='divlist" + listnumber + "' class='report_data'>");
                            ...
                            arr.push(checkStringUndefined(res.list[i].pcustomerid));
                            arr.push("</div>");
                        ...
                        if (res.colmap.sendcostamount == "sendcostamount") {
                            ...
                            arr.push(checkNumberUndefined(res.list[i].sendcostamount));
                            arr.push("</div>");
                        }
                    ...
                    $("#report_footer_salesCustomerReportPage").html('');
                    var arr1 = new Array();
                    for (var i in res.footer) {
                        ...
                        if (res.colmap.sendcostamount == "sendcostamount") {
                            ...
                            arr1.push(checkNumberUndefined(res.footer[i].sendcostamount));
                            arr1.push("</div>");
                        }
                    ...
                    $('#report_footer_salesCustomerReportPage').append(arr1.join(''));
                    initDivData();//初始化隐藏div
                    showHeader();//显示每条数据头
                    showDataDiv("divlist","div");//将div排列成左右格式
                    nextpagenumber=listnumber;
                }else{
                    var footerHtml=$("#report_footer_salesCustomerReportPage").html();
    ...
    function showFooterData(){
        layer.open({
            content: document.getElementById("report_footer_salesCustomerReportPage").outerHTML,
            btn: ['确定']
        });
    }
    ...
    function showDataByCustomer(customerid,customername) {
        var businessdate1 = $("#businessdate1").val();
        var businessdate2 = $("#businessdate2").val();
//        location.hash = "pagethree";
        location.href="phone/showSalesCustomerDetailPage.do?customerid="+customerid+"&customername="+customername+"&businessdate1="+
                businessdate1+"&businessdate2="+businessdate2;
    }
</script>
</body>
</html>

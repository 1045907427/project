...
<head>
    ...
    <style>
        ...
        #report_footer_salesDemandReportDataPage{
            text-align: left;;
        }
        .divhead{
...
<div data-role="page" id="pagetwo">
    <form method="post"  class="form-horizontal" role="form" id="report_form_salesDemandReportDataPage" data-ajax="false">
        ...
        <input type="hidden" id="status" name="status" value="${paramterMap.status}"/>
    ...
    <div data-role="header" data-position="fixed">
        ...
        <div data-role="popup" id="myPopup" style="width: 100% ;height:90%;display: none;">
            <div>
                <ul data-role="listview" data-inset="true" id="report_footer_salesDemandReportDataPage">
                </ul>
            </div>
        </div>
...
<script type="text/javascript">
    function searchData() {
        ...
        $('#thelist').html('');
        $("#report_footer_salesDemandReportDataPage").html('');
        i=0;
        listnumber=0;
        nextpagenumber=0;
    ...
    var initQueryJSON = $("#report-query-form-salesDemandReportDataPage").serializeJSON();
    $(function(){
        searchData();
    });
    var listnumber=0;
...
//        }
    ...
    function Ajax(page){ // ajax后台交互
        ...
        $.ajax({
            type : "post",
            dataType : "JSON",
            url : "phone/getSalesDemandReportData.do", // 你请求的地址
            data : $("#report_form_salesDemandReportDataPage").serialize(),
            success : function(res){
                if(res.list.length){ // 如果后台传过来有数据执行如下操作 ， 没有就执行else 告诉用户没有更多内容呢
                    ...
                    for (var i in res.list) {
                        ...
                        arr.push(checkNumberUndefined(res.list[i].taxprice));
                        arr.push("</div>");
                        arr.push("<div class='unitnum'>数量:");
                        arr.push(formatterBigNumNoLen(res.list[i].unitnum));
                        arr.push("</div>");
                        arr.push("<div class='taxamount'>金额:");
                        arr.push(checkNumberUndefined(res.list[i].taxamount));
                        arr.push("</div>");
                        arr.push("</div>");
                    ...
                    $('#report_footer_salesDemandReportDataPage').append(arr1.join(''));
                    ...
                    var footerHtml=$("#report_footer_salesDemandReportDataPage").html();
...
//                    }
    ...
    function showFooterData(){
        layer.open({
            content: document.getElementById("report_footer_salesDemandReportDataPage").outerHTML,
            btn: ['确定']
        });
    }

...
<html>
  <head>
    <title>费用科目新增页面</title>
  </head>
  
  <body>
  	<form action="" id="journalsheet-form-deptcostssubjectAddPage" method="post">
  		<input type="hidden" name="parentId" id="journalsheet-parentId-deptcostssubjectAddPage" value="<c:out value="${deptCostsSubject.id }"></c:out>" />
  		<input type="hidden" name="parentName" id="journalsheet-parentName-deptcostssubjectAddPage" value="<c:out value="${deptCostsSubject.name }"></c:out>" />
	    <div class="pageContent" style="width:500px;">
	    	...
	    	<p>
	    		<label>上级分类：</label>
	    		<input type="text" id="journalsheet-parent-deptcostssubjectAddPage" value="<c:out value="${deptCostsSubject.id }"></c:out>" name="deptCostsSubject.pid" />
	    	</p>
	    	<p style="height:auto;width:auto;">
	    		<label>备注：</label>
	    		<textarea rows="3" cols="50" class="easyui-validatebox" style="width: 195px;" name="deptCostsSubject.remark" validType="maxLen[200]"></textarea>
	    	</p>
	    	<p>
	    		<label>状态：</label>
	    		<select class="easyui-combobox" disabled="disabled" style="width:200px;"  name="deptCostsSubject.state">
    ...
    <script type="text/javascript">
    	...
    	function addDeptCostsSubject(type){
    		if(type == "save"){
        		if(!$("#journalsheet-form-deptcostssubjectAddPage").form('validate')){
        			return false;
        		}
        	}
        	var ret = deptCostsSubject_ajaxContent($("#journalsheet-form-deptcostssubjectAddPage").serializeJSON(),'journalsheet/costsFee/addDeptCostsSubject.do?type='+type);
        	...
        	if(retJson.flag){
        		...
        		var id = $("#journalsheet-thisId-deptcostssubject").val();
	  		    ...
	  		    refreshLayout("任务【详情】",'journalsheet/costsFee/showDeptCostsSubjectViewPage.do?id='+$("#journalsheet-id-deptcostssubjectAddPage").val());
        	}
        	else{
        		if(type == "save"){

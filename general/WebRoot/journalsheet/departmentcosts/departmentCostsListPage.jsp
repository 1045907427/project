  ...
  <body>
    <div class="easyui-layout" data-options="fit:true">
    	<div data-options="region:'north'" style="height: 30px;">
    		<div id="departmentCosts-button-deptmentCosts"></div>
    	</div>
    	<div data-options="region:'center',border:false" style="border: 0px;">
    		<div id="departmentCosts-table-deptmentCostsBtn">
    			<form action="" id="departmentCosts-form-ListQuery" method="post" style="padding-left: 5px; padding-top: 2px;">
	    			<table cellpadding="0" cellspacing="1" border="0">
	    				<tr>
	    					<td>业务日期:</td>
	    					<td><input id="begintime" name="begintime" value="${yearfirstmonth }"  class="Wdate" style="width:80px;" onfocus="WdatePicker({'dateFmt':'yyyy-MM',maxDate:'${yearcurmonth }'})"/>
   								到<input id="endtime" name="endtime" value="${yearcurmonth }"  class="Wdate" style="width:80px;" onclick="WdatePicker({'dateFmt':'yyyy-MM',maxDate:'${yearcurmonth }'})"  />
	    					</td>
	    					<td>所属部门:</td>
    		...
    		<table id="departmentCosts-table-deptmentCosts"></table>
    		<div style="display:none">
	    		<div id="departmentCosts-dialog-add-operate"></div>
	    		<div id="departmentCosts-dialog-edit-operate"></div>
	    		<div id="departmentCosts-dialog-view-operate"></div>
    ...
    <script type="text/javascript">
    	...
    	var costsFeeListColJson=$("#departmentCosts-table-deptmentCosts").createGridColumnLoad({
	    ...
	    function showDeptCostsInitAddDialog(title, url){
	   		$('#departmentCosts-dialog-add-operate').dialog({  
			    title: title,  
			    width: 450,  
			    height: 180,  
			    closed: true,
			...
			$('#departmentCosts-dialog-add-operate').dialog('open');
	   	}
	    function showDeptCostsEditDialog(title, url){
	   		$('#departmentCosts-dialog-edit-operate').dialog({  
			    title: title,  
			    width: 680,  
			    height: 450,  
			    closed: true,
			...
			$('#departmentCosts-dialog-edit-operate').dialog('open');
	   	}

	    function showDeptCostsViewDialog(title, url){
	   		$('#departmentCosts-dialog-view-operate').dialog({  
			    title: title,  
			    width: 680,  
			    height: 450,  
			    closed: false,  
			...
			$('#departmentCosts-dialog-view-operate').dialog('open');
	   	}
		...
		$("#departmentCosts-query-advanced").advancedQuery({
			//查询针对的表
	 		name:'t_js_departmentcosts',
	 		//查询针对的表格id
	 		datagrid:'departmentCosts-table-deptmentCosts'
		});
		  ...
		  //所属部门查询
			...
			$("#departmentCosts-query-List").click(function(){
				//把form表单的name序列化成JSON对象
	      		...
	      		$("#departmentCosts-table-deptmentCosts").datagrid({
			...
			$("#departmentCosts-query-reloadList").click(function(){
				...
				$("#departmentCosts-table-deptmentCosts").datagrid('loadData',{total:0,rows:[]});
			...
			$("#departmentCosts-button-deptmentCosts").buttonWidget({
     			//初始默认按钮 根据type对应按钮事件
				initButton:[
					{}
	 			],
				buttons:[
					...
					<security:authorize url="/journalsheet/costsFee/departmentCostsEditBtn.do">
					{
						...
						iconCls:'button-edit',
						handler:function(){
							var dataRow=$("#departmentCosts-table-deptmentCosts").datagrid('getSelected');
							if(dataRow==null){
								$.messager.alert("提醒","请选择相应的部门费用!");
								return false;
							}
							showDeptCostsEditDialog("部门费用【修改】", 'journalsheet/costsFee/showDepartmentCostsEditPage.do?id='+dataRow.id);
						}
					},
					</security:authorize>
					<security:authorize url="/journalsheet/costsFee/auditDepartmentCostsMore.do">
					{
						...
						handler: function(){
							var rows =  $("#departmentCosts-table-deptmentCosts").datagrid('getChecked');
				  				...
				  				$.ajax({
							        ...
							        success:function(json){
							        	...
							        	if(json.flag==true){
											...
											$("#departmentCosts-table-deptmentCosts").datagrid('reload');
											$("#departmentCosts-table-deptmentCosts").datagrid('clearSelections');	
						  		        }
						  		        else{
						...
						handler: function(){
							$("#departmentCosts-buttons-importclick").Excel('import',{
								...
								onClose: function(){ //导入成功后窗口关闭时操作，
							        var queryJSON = $("#departmentCosts-form-ListQuery").serializeJSON();
							        $("#departmentCosts-table-deptmentCosts").datagrid("load",queryJSON);
								}
							});
					...
					<security:authorize url="/journalsheet/costsFee/showDeptSupplierCostBtn.do">
					{
						id:'button-search-suppliercost',
						name:'查看供应商摊派',
						iconCls:'icon-search',
						handler: function(){
							var row =  $("#departmentCosts-table-deptmentCosts").datagrid('getSelected');
							if(row==null || row.id==""){
     		...
     		$("#departmentCosts-table-deptmentCosts").datagrid({ 
				...
				columns:costsFeeListColJson.common,
		 		...
		 		selectOnCheck:true,
				pageSize:20,
				toolbar:'#departmentCosts-table-deptmentCostsBtn',
				pageList:[10,20,30,50,200],
			    onSelect:function(rowIndex, rowData){
			    	if(rowData.status=="3"){
			    		$("#departmentCosts-button-deptmentCosts").buttonWidget("disableButton", 'button-id-edit');
			    		$("#departmentCosts-button-deptmentCosts").buttonWidget("disableButton", 'button-id-delete');
			    		$("#departmentCosts-button-deptmentCosts").buttonWidget("disableButton", 'button-id-audit');
			    		$("#departmentCosts-button-deptmentCosts").buttonWidget("enableButton", 'button-id-oppaudit');
			    	}else{
			    		$("#departmentCosts-button-deptmentCosts").buttonWidget("enableButton", 'button-id-edit');
			    		$("#departmentCosts-button-deptmentCosts").buttonWidget("enableButton", 'button-id-delete');
			    		$("#departmentCosts-button-deptmentCosts").buttonWidget("enableButton", 'button-id-audit');
			    		$("#departmentCosts-button-deptmentCosts").buttonWidget("disableButton", 'button-id-oppaudit');
		    	...
		    	onDblClickRow:function(rowIndex, rowData){
			    	<%if(isEdit){%>
			    		if(rowData.status!='3'){
			    			showDeptCostsEditDialog("部门费用【修改】", 'journalsheet/costsFee/showDepartmentCostsEditPage.do?id='+rowData.id);
			    		}else{
			    			showDeptCostsViewDialog("部门费用【详情】", 'journalsheet/costsFee/showDepartmentCostsViewPage.do?id='+rowData.id);
			    		}
	     			<% }else { %>
	     				showDeptCostsViewDialog("部门费用【详情】", 'journalsheet/costsFee/showDepartmentCostsViewPage.do?id='+rowData.id);
	     			<%}%>
		...
		function deptCostsCountTotalAmount(){
    		...
    		if(null==rows || rows.length==0){
    			...
    			$("#departmentCosts-table-deptmentCosts").datagrid("reloadFooter",foot);
           		return false;
       		}

  ...
  <body>
  	<div class="easyui-layout" data-options="fit:true">
  		<div data-options="region:'center',border:false">
            <div id="sales-div-orderGoodsPromotionGiveAddPage">
  			<form id="sales-form-orderGoodsPromotionGiveAddPage">
                <table cellpadding="5" cellspacing="5">
                    ...
                    <tr>
                        <td>数量</td>
                        <td><input id="sales-unitnum-orderGoodsPromotionGiveAddPage" class="formaterNum easyui-validatebox len150 goodsNum" value="0" name="unitnum"  data-options="required:true,validType:'intOrFloatNum[${decimallen}]'" /></td>
                        <td>辅数量：</td>
                        <td><input id="sales-auxnum-orderGoodsPromotionGiveAddPage" name="auxnum" class="formaterNum easyui-validatebox goodsNum" value="0" style="width:60px;" data-options="validType:'integer'" /><span id="sales-auxunitname-orderGoodsPromotionGiveAddPage">${promotionPackageGroupDetail.auxunitname}</span>
                            <input id="sales-overnum-orderGoodsPromotionGiveAddPage" name="overnum" class="formaterNum easyui-validatebox goodsNum" value="0" style="width:60px;" data-options="validType:'intOrFloatNum[${decimallen}]'" /><span id="sales-unitname-orderGoodsPromotionGiveAddPage">${promotionPackageGroupDetail.unitname}</span>
                        </td>
                    </tr>
                    <tr>
                        <td>含税单价：</td>
            ...
            <h1>赠送商品
                <c:if test="${promotionPackageGroup.remainnum>0}">
  		...
  		<div data-options="region:'south',border:false">
  			<div class="buttonDetailBG" style="height:30px;text-align:right;">
	  			<input type="button" value="保存" name="savegoon" id="sales-savegoon-orderGoodsPromotionGiveAddPage" />
  			</div>
  		</div>
  	</div>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $("#sales-savegoon-orderGoodsPromotionGiveAddPage").click(function(){
                setTimeout(updateSaveGiveNumDetail(),200);
            });
        });
        function giveunitnumChange(type){ //数量改变方法
            ...
            if(type == 1){
                ...
                data = {id:goodsId,unitnum:unitnum,taxtype:taxtype,price:price,groupid:groupid,orderid:orderid};
            }
            else if(type == 2){
                url = "sales/getUnitNumAndAmountByGive.do"
                data = {id:goodsId,auxnum:auxnum,overnum:overnum,taxtype:taxtype,price:price,groupid:groupid,orderid:orderid};
            }
            $.ajax({
                url:url,
                dataType:'json',
        ...
        //保存买赠
        function updateSaveGiveNumDetail(){
            var groupid = $("#sales-groupid-orderGoodsPromotionGiveAddPage").val();
                    ...
                    for(var i=0;i<rows.length;i++){
                        ...
                        if(unitnum>0){
                            ...
                            for(var i=0;i<addrows.length;i++){
                                if(addrows[i].goodsid == data.goodsid && addrows[i].groupid==data.groupid
                                        && addrows[i].deliverytype==data.deliverytype && addrows[i].taxprice==data.taxprice){
                                    $("#sales-datagrid-orderAddPage").datagrid('deleteRow', i);
                                    break;
                                }
                            }
                        }

KindEditor.plugin('zradio', function(K) {
	var self = this, name = 'zradio';
	self.plugin.textDialog = function(options) {
		var html = [
			...
			'</div>',
			'<div class="ke-dialog-row">',
			'<label style="width:90px;float:left;">单选框选项</label>',
			'<div style="float:left;height:120px;width:320px;overflow:auto;">',
			'<dl class="select-item">',
			'<dd><a href="javascript:;" class="addRow">添加</a> <input type="checkbox" name="checkfirst" />默认选中第一项</dd>',
			...
			yesBtn : {
				...
				click : function(e){
					...
					var desc = descBox.val(),
						...
						wunit = wBox.val(),
						hunit = hBox.val(),
						checkfirst = $("input[name=checkfirst]").attr("checked");
					var style = '';
					if(width !== ''){
					...
					for(var i=0;i<values.length;i++){
						if(checkfirst == "checked" && i==0){
							html += '<input type="radio" id="r_'+i+'" '+p+' value="'+values[i]+'" checked="checked" /><label for="r_'+i+'">'+texts[i]+'</label>';
						}
						else{
							html += '<input type="radio" id="r_'+i+'" '+p+' value="'+values[i]+'" /><label for="r_'+i+'">'+texts[i]+'</label>';
						}
					}

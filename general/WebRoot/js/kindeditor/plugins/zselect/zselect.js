KindEditor.plugin('zselect', function(K) {
	var self = this, name = 'zselect';
	self.plugin.textDialog = function(options) {
		var html = [
			...
			'</div>',
			'<div class="ke-dialog-row">',
			'<label style="width:90px;float:left;">下拉框选项</label>',
			...
			yesBtn : {
				...
				click : function(e){
					...
					for(var i=0;i<values.length;i++){
						options += '<option value="'+values[i]+'">'+texts[i]+'</option>';
					}
					html += options;
					html += '</select>';
		...
		$(".deleteRow").live('click', function(){alert("123");
			//$(this).parent().remove();
		});
		return dialog;
	};

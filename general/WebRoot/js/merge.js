        ...
        if(ids){
            ...
            if (mapPoint){
                ...
                if(!shift && !ctrl && !alt) { // No Modifiers
                    ...
                    } else if(character&& mapPoint[character]) {
...
//查找单个汉字首字母
function arraySearchFirstPinYin(l1,l2){
	for (var name in PinYin){
		if (PinYin[name].indexOf(l1)!=-1) {
			return ucfirstchar(name); break;
		}
	}
	return false;
}
//获取整个汉字首字母组合
function convertToFirstPinyinLower(l1){
	var l2 = l1.length;
	var I1 = "";
	var reg = new RegExp('[a-zA-Z0-9\- ]');
	for (var i=0;i<l2;i++) {
		var val = l1.substr(i,1);
		var name = arraySearchFirstPinYin(val,PinYin);
		if(reg.test(val)) {
			I1 += val;
		} else if (name!==false) {
			I1 += name;

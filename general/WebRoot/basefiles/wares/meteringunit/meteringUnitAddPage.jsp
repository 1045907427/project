...
<html>
  <head>
    <title>计量单位新增页面</title>
  </head>
  
  <body>
    <form action="" method="post" id="meteringUnit-form-addMeteringUnit">
    	<table cellpadding="2" cellspacing="2" border="0">
    		...
    		<tr>
    			<td>名称:</td>
    			<td><input name="meteringUnit.name" class="easyui-validatebox" validType="validName[20]" style="width: 200px;"/></td>
    		</tr>
    		<tr>
    			<td>备注:</td>
    			<td><textarea name="meteringUnit.remark" class="easyui-validatebox" validType="maxLen[200]" style="height:80px;width: 195px;overflow: hidden"></textarea></td>
    		</tr>
    		<tr>
    			<td>状态:</td>
	...
	function addMeteringUnitHold(){
		...
		$("#meteringUnit-form-addMeteringUnit").submit();
	}
	
	$("#meteringUnit-form-addMeteringUnit").form({
  	...
  	function addMeteringUnitSave(){
		...
		var ret = meteringUnit_AjaxConn($("#meteringUnit-form-addMeteringUnit").serializeJSON(),'basefiles/addMeteringUnitSave.do');
		var retJson = $.parseJSON(ret);
		if(retJson.flag){
			$("#meteringUnit-table-list").datagrid('reload');
			refreshLayout('计量单位【详情】', 'basefiles/showMeteringUnitInfoPage.do?id='+$("#meteringUnit-input-id").val());

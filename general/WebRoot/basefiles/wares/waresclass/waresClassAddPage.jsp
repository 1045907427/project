  ...
  <body>
    <form action="" method="post" id="waresClass-form-add">
    	<table cellpadding="2" cellspacing="2" border="0">
    		<tr>
    			<td>本级编码:</td>
    			<td><input id="waresClass-input-thisid" type="text" name="waresClass.thisid" class="easyui-validatebox" onchange="getInputValue(this.value)" validType="validLen[${nextLen }]" required="true"  style="width:200px;"/>
				    <input type="hidden" id="waresClass-hidden-hdpId" value="<c:out value="${waresClass.id }"></c:out>"/>
				    <input type="hidden" id="waresClass-hidden-hdpName" value="<c:out value="${waresClass.name }"></c:out>"/>
    		...
    		<tr>
    			<td>名称:</td>
    			<td><input type="text" id="waresClass-input-name" name="waresClass.name" value="<c:out value="${waresClass.name}"></c:out>" class="easyui-validatebox" required="true" validType="maxLen[50]" style="width:200px;"/></td>
    		</tr>
    		<tr>
    			<td>上级分类:</td>
    			<td>
    				<input id="waresClass-widget-pid" type="text" name="waresClass.pid" value="<c:out value="${waresClass.id }"></c:out>"/>
    			</td>
    		</tr>
    		<tr>
    			<td>备注:</td>
    			<td><textarea name="waresClass.remark" class="easyui-validatebox" validType="maxLen[200]" style="height:40px;width: 195px;overflow: hidden"></textarea></td>
    		</tr>
    		<tr>
    			<td>状态:</td>
    ...
    <script type="text/javascript">
		...
		function addWaresClassHold(){
			...
			var ret = waresClass_AjaxConn($("#waresClass-form-add").serializeJSON(),'basefiles/addWaresClassHold.do','提交中..');
		...
		function addWaresClassSave(){
			if(!$("#waresClass-form-add").form('validate')){
				return false;
			}
			$("#waresClass-hidden-hdState").val(2);
			var ret = waresClass_AjaxConn($("#waresClass-form-add").serializeJSON(),'basefiles/addWaresClassSave.do','提交中..');
			var retJson = $.parseJSON(ret);
			if(retJson.flag){
				var id = $("#wares-thisId-waresClass").val();
	  		    var pid = $("#wares-pId-waresClass").val();
			...
			$.extend($.fn.validatebox.defaults.rules, {
	   			validLen:{
	   				validator:function(value,param){
	   					...
	   					if(reg.test(value)){
	   						var ret=waresClass_AjaxConn({id:$("#waresClass-hidden-hdpId").val()+value},"basefiles/isRepeatWCID.do");//判断编码是否重复，true 不重复，false 重复
	   						...
	   						$("#waresClass-button-layoutTree").buttonWidget("enableMenuItem","button-hold");
	   						$("#waresClass-button-layoutTree").buttonWidget("enableMenuItem","button-save");
	   						$.fn.validatebox.defaults.rules.validLen.message ='请输入{0}位字符!';
	   						return false;
	   					}
	   				},
	   				message:''

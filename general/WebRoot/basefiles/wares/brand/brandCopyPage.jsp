...
<html>
  <head>
    <title>商品品牌复制页面</title>
  </head>
  
  <body>
    <form action="" method="post" id="wares-form-brandCopy">
    	<table cellpadding="2" cellspacing="2" border="0">
    		<tr>
    			...
    			<td>
    				...
    				<input id="wares-hidden-copyType" name="type" type="hidden"/>
    				<input name="brand.state" type="hidden" value="${brand.state }"/>
    				<input name="brand.oldId" type="hidden" value="<c:out value="${brand.id }"></c:out>"/>
    			</td>
    		</tr>
    		<tr>
    			<td>名称:</td>
    		...
    		<tr>
    			<td>所属供应商:</td>
    			<td><input id="wares-widget-Supplier" name="brand.supplierid" value="<c:out value="${brand.supplierid }"></c:out>"/></td>
    		</tr>
            <tr>
                <td>默认税种:</td>
    ...
    <script type="text/javascript">
		...
		function addBrand(type){
			$("#wares-hidden-copyType").val(type);
			if(type == "save"){
				if(!$("#wares-form-brandCopy").form('validate')){
					return false;
				}
			}
			var ret = brand_AjaxConn($("#wares-form-brandCopy").serializeJSON(),'basefiles/addBrand.do');
			var retJson = $.parseJSON(ret);
			if(retJson.flag){

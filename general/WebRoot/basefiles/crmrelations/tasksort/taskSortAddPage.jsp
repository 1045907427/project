...
<html>
  <head>
    <title>任务分类添加页面</title>
  </head>
  
  <body>
    <form action="" id="taskSort-form-add" method="post">
    	<input type="hidden" name="parentId" id="crmrelations-parentId-taskSortAdd" value="<c:out value="${taskSort.id }"></c:out>" />
  		<input type="hidden" name="parentName" id="crmrelations-parentName-taskSortAdd" value="<c:out value="${taskSort.name }"></c:out>" />
  		<div class="pageContent" style="width:500px;">
	    	...
	    	<p>
	    		<label>上级分类：</label>
	    		<input type="text" id="crmrelations-parent-taskSort" value="<c:out value="${taskSort.id }"></c:out>" name="taskSort.pid" />
	    	</p>
	    	<p>
	    		<label>默认责任部门：</label>
	    		<input type="text" id="crmrelations-defaultDept-taskSort" name="taskSort.defaultdeptid" />
	    	</p>
	    	<p>
	    		<label>默认责任人：</label>
	    		<input type="text" id="crmrelations-defaultUser-taskSort" name="taskSort.defaultuserid"/>
	    	</p>
	    	<p>
	    		<label>默认费用分类：</label>
	    		<input type="text" id="crmrelations-defaultexpenses-taskSort" name="taskSort.defaultexpensesid"/>
	    	</p>
	    	<p style="height:auto;width:auto;">
	    		<label>备注：</label>
	    		<textarea rows="3" cols="50" style="width: 200px;" name="taskSort.remark"></textarea>
	    	</p>
	    	<p>
	    		<label>状态：</label>
    ...
    <script type="text/javascript">
    	...
    	function addTaskSort(type){
        	if(type == "save"){
        		if(!$("#taskSort-form-add").form('validate')){
        			return false;
        		}
        	}
        	var ret = taskSort_AjaxConn($("#taskSort-form-add").serializeJSON(),'basefiles/crmrelations/addTaskSort.do?type='+type,'提交中..');
    		...
    		$("#crmrelations-parent-taskSort").widget({
    			...
    			onChecked: function(data, check){
    				if(check){
    					...
    					validLengthAndUsed(taskSort_lenArr[(data.level + 1)], "basefiles/crmrelations/isUsedTaskSortID.do", $("#crmrelations-parentId-taskSortAdd").val(), '', "该编号已被使用，请另输编号！"); //验证输入长度
    				}
    				else{
    					...
    					$("#crmrelations-buttons-taskSortAdd").buttonWidget("enableButton", "button-hold");
    					$("#crmrelations-buttons-taskSortAdd").buttonWidget("enableButton", "button-save");
    					validLengthAndUsed(taskSort_lenArr[0], "basefiles/taskSortAddNOUsed.do", $("#crmrelations-parentId-taskSortAdd").val(), '', "该编号已被使用，请另输编号！"); //验证输入长度
    				}
    			},
    			onClear: function(){
    				$("#crmrelations-id-taskSortAdd").val($("#crmrelations-thisId-taskSortAdd").val());
    				...
    				$("#crmrelations-buttons-taskSortAdd").buttonWidget("enableButton", "button-hold");
    				$("#crmrelations-buttons-taskSortAdd").buttonWidget("enableButton", "button-save");
    				validLengthAndUsed(taskSort_lenArr[0], "basefiles/crmrelations/isUsedTaskSortID.do", $("#crmrelations-parentId-taskSortAdd").val(), '', "该编号已被使用，请另输编号！"); //验证输入长度
    			}
    		});

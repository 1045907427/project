...
<html>
  ...
  <body>
  	<form action="" id="CRM-form-marketActivitySortAddPage" method="post">
  		<input type="hidden" name="parentId" id="CRM-parentId-marketActivitySortAddPage" value="<c:out value="${marketActivitySort.id }"></c:out>" />
  		<input type="hidden" name="parentName" id="CRM-parentName-marketActivitySortAddPage" value="<c:out value="${marketActivitySort.name }"></c:out>" />
	    <div class="pageContent" style="width:500px;">
	    	...
	    	<p>
	    		<label>上级分类：</label>
	    		<input type="text" id="CRM-parent-marketActivitySortAddPage" value="<c:out value="${marketActivitySort.id }"></c:out>" name="marketActivitySort.pid" />
	    	</p>
	    	<p style="height:auto;width:auto;">
	    		<label>备注：</label>
	    		<textarea rows="3" cols="50" style="width: 200px;" name="marketActivitySort.remark"></textarea>
	    	</p>
	    	<p>
	    		<label>状态：</label>
    ...
    <script type="text/javascript">
    	function addMarketActivitySort(type){
    		if(type == "save"){
        		if(!$("#CRM-form-marketActivitySortAddPage").form('validate')){
        			return false;
        		}
        	}
        	var ret = marketActivitySort_ajaxContent($("#CRM-form-marketActivitySortAddPage").serializeJSON(),'basefiles/crmrelations/addmarketActivitySort.do?type='+type,'提交中..');

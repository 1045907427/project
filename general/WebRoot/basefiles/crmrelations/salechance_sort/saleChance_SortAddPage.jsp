...
<html>
  ...
  <body>
  	<form action="" id="saleChance-form-add" method="post">
  		<input type="hidden" name="parentId" id="crmrelations-parentId-saleChance" value="<c:out value="${saleChance_Sort.id }"></c:out>" />
  		<input type="hidden" name="parentName" id="crmrelations-parentName-saleChance" value="<c:out value="${saleChance_Sort.name }"></c:out>" />
	    <div class="pageContent" style="width:500px;">
	    	...
	    	<p>
	    		<label>上级分类：</label>
	    		<input type="text" id="crmrelations-parent-saleChance" value="<c:out value="${saleChance_Sort.id }"></c:out>" name="saleChance_Sort.pid" />
	    	</p>
	    	<p style="height:auto;width:auto;">
	    		<label>备注：</label>
	    		<textarea rows="3" cols="50" style="width: 200px;" name="saleChance_Sort.remark"></textarea>
	    	</p>
	    	<p>
	    		<label>状态：</label>
	    		<select class="easyui-combobox" disabled="disabled" style="width:206px;"  name="saleChance_Sort.state">
    ...
    <script type="text/javascript">
        function addSaleChance(type){
        	if(type == "save"){
        		if(!$("#saleChance-form-add").form('validate')){
        			return false;
        		}
        	}
        	var ret = saleChanceSort_AjaxConn($("#saleChance-form-add").serializeJSON(),'basefiles/crmrelations/addSaleChance.do?type='+type,'提交中..');
    	...
    	$(function(){
			...
			$("#crmrelations-buttons-saleChance").buttonWidget("initButtonType", 'add');
    	});
    </script>
  </body>
</html>

...
<html>
  <head>
    <title>采购区域修改页面</title>
  </head>
  
  <body>
  	<form action="basefiles/updateBuyArea.do" id="buy-form-buyAreaAddPage" method="post">
  		<input type="hidden" name="addType" id="buy-addType-buyAreaAddPage" />
  		<input type="hidden" name="parentId" id="buy-parentId-buyAreaAddPage" value="<c:out value="${buyArea.pid }"></c:out>" />
  		<input type="hidden" name="parentName" id="buy-parentName-buyAreaAddPage" value="<c:out value="${parentName }"></c:out>" />
  		<input type="hidden" id="buyArea-oldid" name="buyArea.oldid" value="<c:out value="${buyArea.id }"></c:out>" />
  		<input type="hidden" id="buyArea-oldpid" value="<c:out value="${buyArea.pid }"></c:out>"/>
  		<input type="hidden" id="buyArea-oldthisid" value="<c:out value="${buyArea.thisid }"></c:out>"/>
  		<input type="hidden" id="buyArea-thisname" value="<c:out value="${buyArea.thisname }"></c:out>"/>
        <input type="hidden" id="buyArea-hidden-hdState" name="buyArea.state" value="${buyArea.state}"/>
	    <div class="pageContent" style="width:500px;">
	    	<p>
	    		<label>本级编号：</label>
	    		<input type="text" class="easyui-validatebox" name="buyArea.thisid" value="<c:out value="${buyArea.thisid }"></c:out>" <c:if test="${editFlag == false }">readonly="readonly"</c:if> id="buy-thisId-buyAreaAddPage" data-options="required:true,validType:['validLength[${len}]']" />
	    	</p>
	    	<p>
	    		<label>编号：</label>
	    	...
	    	<p>
	    		<label>本级名称：</label>
	    		<input type="text" class="easyui-validatebox" name="buyArea.thisname" value="<c:out value="${buyArea.thisname }"></c:out>" id="buy-thisName-buyAreaAddPage" required="true" validType="isRepeatName[20]"/>
	    	</p>
	    	<p>
	    		<label>名称：</label>
	    	...
	    	<p>
	    		<label>上级分类：</label>
	    		<input type="text" id="buy-parent-buyAreaAddPage" value="<c:out value="${buyArea.pid }"></c:out>" name="buyArea.pid" <c:if test="${editFlag == false }">disabled="disabled"</c:if> />
	    	</p>
    ...
    <script type="text/javascript">
    	$(function(){
    		$("#buy-buttons-buyArea").buttonWidget("setDataID", {id:$("#buyArea-oldid").val(),state:'${buyArea.state}',type:'edit'});
    		validLengthAndUsed('${len}', "basefiles/isBuyAreaIdExist.do", $("#buy-parentId-buyAreaAddPage").val(), $("#buyArea-oldthisid").val(), "该编号已被使用，请另输编号！"); //验证输入长度
    		
    		$.extend($.fn.validatebox.defaults.rules, {
    			isRepeatName:{//true 不重复，false 重复
	   				validator:function(value,param){
	   					if(value.length <= param[0]){
	   						if($("#buyArea-thisname").val() == value){
	   							return true;
	   						}
    		...
    		$("#buy-parent-buyAreaAddPage").widget({
    			...
    			onChecked: function(data, check){
    				if(check){
    					...
    					$("#buy-buttons-buyArea").buttonWidget("enableButton", "button-hold");
    					$("#buy-buttons-buyArea").buttonWidget("enableButton", "button-save");
    					validLengthAndUsed(buyArea_lenArr[0], "basefiles/isBuyAreaIdExist.do", $("#buy-parentId-buyAreaAddPage").val(), $("#buyArea-oldthisid").val(), "该编号已被使用，请另输编号！"); //验证输入长度
    				}
    			},
    			onClear: function(){
    				$("#buy-id-buyAreaAddPage").val($("#buy-thisId-buyAreaAddPage").val());

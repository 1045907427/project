...
<html>
  ...
  <body>
  	<form action="" id="finance-form-expensesSortCopyPage" method="post">
	    ...
	    <div class="pageContent" style="width:500px;">
	    	...
	    	<p>
	    		<label>编号：</label>
	    		<input type="text" class="easyui-validatebox" readonly="readonly" value="<c:out value="${expensesSort.pid }"></c:out>" name="expensesSort.id" id="finance-id-expensesSortAddPage" />
	    	</p>
	    	<p>
	    		<label>本级名称：</label>
	    	...
	    	<p>
	    		<label>名称：</label>
	    		<input type="text" class="easyui-validatebox" readonly="readonly" value="<c:out value="${parentName }"></c:out>" name="expensesSort.name" id="finance-name-expensesSortAddPage" />
	    	</p>
	    	<p>
	    		<label>上级分类：</label>
	    		<input type="text" id="finance-parent-expensesSortAddPage" value="<c:out value="${expensesSort.pid }"></c:out>" name="expensesSort.pid" />
	    	</p>
	    	<p>
	    		<label>对应会计科目：</label>
	    	...
	    	<p style="height:auto;width:auto;">
	    		<label>备注：</label>
	    		<textarea rows="3" cols="50" class="easyui-validatebox" style="width: 200px;" name="expensesSort.remark" validType="maxLen[200]"><c:out value="${expensesSort.remark }"></c:out></textarea>
	    	</p>
	    	<p>
	    		<label>状态：</label>
	    		<select class="easyui-combobox" disabled="disabled" style="width:206px;"  name="expensesSort.state">
    ...
    <script type="text/javascript">
    	...
    	function addExpensesSort(type){
    		if(type == "save"){
        		if(!$("#finance-form-expensesSortCopyPage").form('validate')){
        			return false;
        		}
        	}
        	var ret = expensesSort_ajaxContent($("#finance-form-expensesSortCopyPage").serializeJSON(),'basefiles/finance/addExpensesSort.do?type='+type);
        	...
        	if(retJson.flag){
        		...
        		var id = $("#finance-thisId-expensesSort").val();
	  		    ...
	  		    refreshLayout("费用【详情】",'basefiles/finance/expensesSortViewPage.do?id='+$("#finance-id-expensesSortAddPage").val());
        	}
        	else{
        		if(type == "save"){

...
UE.registerUI('numericDialog', function(editor, uiName) {
    //创建dialog
    var dialog = new UE.ui.Dialog({
        //指定弹出层中页面的路径，这里只能支持页面,因为跟addCustomizeDialog.js相同目录，所以无需加路径
        iframeUrl:formData.ctx + '/common/form/numeric.html',
        //需要指定当前的编辑器实例
        editor:editor,
        //指定dialog的名字
        name:uiName,
    ...
    var btn = new UE.ui.Button({
        ...
        title:'dialogbutton' + uiName,
        //需要添加的额外样式，指定icon图标，这里默认使用一个重复的icon
        cssRules :'background: url(' + formData.ctx + '/styles/images/field/field_numeric.png) no-repeat 2px 3px!important;',
        onclick:function () {
            //渲染dialog
            dialog.render();
            dialog.open();

...
<html>
  <head>
    <title>客户应收款冲差修改页面</title>
  </head>
  
  <body>
   	<form action="account/payable/UpdatePurchaseInvoicePush.do" method="post" id="account-form-purchaseInvoicePush">
   		<table  border="0" style="width: 380px;">
   			<tr>
   				...
   				<td style="text-align: left;">
                    ...
                    <input type="hidden" name="purchaseInvoicePush.id" style="width: 200px;" <c:if test="${purchaseInvoicePush!=null}">value="${purchaseInvoicePush.id}"</c:if>/>
   				</td>
   			</tr>
   			<tr>
   			...
   			<tr>
   				...
   				<td style="text-align: left;">
   					<input id="account-purchaseInvoicePush-brand" name="purchaseInvoicePush.brand" style="width: 200px;" <c:if test="${purchaseInvoicePush!=null}">value="${purchaseInvoicePush.brand}"</c:if>/>
   				    <input type="hidden" name="purchaseInvoicePush.status" <c:if test="${purchaseInvoicePush!=null}">value="${purchaseInvoicePush.status}"</c:if>/>
                </td>
   			</tr>
   			<tr>
   				<td style="text-align: right;width: 100px;">冲差金额:</td>
   				<td style="text-align: left;">
   					<input type="text" id="account-purchaseInvoicePush-amount" name="purchaseInvoicePush.amount" style="width: 200px;" class="easyui-validatebox easyui-numberbox" data-options="precision:2,required:true,validType:'intOrFloat'" <c:if test="${purchaseInvoicePush!=null}">value="${purchaseInvoicePush.amount}"</c:if>/>
   				</td>
   			</tr>
   			<tr>
   				<td style="text-align: right;width: 100px;">备注:</td>
   				<td style="text-align: left;">
   					<input type="text" name="purchaseInvoicePush.remark" style="width: 200px;" id="account-purchaseInvoicePush-remark" value="${purchaseInvoicePush.remark}" />
   				    <input type="hidden" name="purchaseInvoicePush.pushtypename" id="account-purchaseInvoicePush-pushtypename" />
                </td>
   			</tr>
   		</table>
    </form>
...
//            }
    		if(pushtype==1){
                $("#account-purchaseInvoicePush-pushtypename").val("代垫费用");
    		}else if(pushtype==2){
                $("#account-purchaseInvoicePush-pushtypename").val("价格冲差");
            }else if(pushtype==3){
                $("#account-purchaseInvoicePush-pushtypename").val("货物冲差");
            }

        ...
        <form id="account-form-goldTaxCustomerInvoiceDetailEditPage" method="post">
            <table style="border-collapse:collapse;" border="0" cellpadding="5" cellspacing="5">
                ...
                <tr id="account-goldTaxCustomerInvoiceDetail-sourcegoods-tr" style="display: none">
                    ...
                    <td><input type="text" id="account-goldTaxCustomerInvoiceDetail-sourcegoodsid" name="sourcegoodsid" style="width:150px;" readonly="readonly"/></td>
                    <td style="text-align: right;">客户商品名称：</td>
                    <td><input type="text" id="account-goldTaxCustomerInvoiceDetail-sourcegoodsname" name="sourcegoodsname" style="width:150px;" readonly="readonly"/></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td><input type="text" id="account-goldTaxCustomerInvoiceDetail-taxprice" name="taxprice" class="easyui-validatebox formaterNum <c:if test="${colMap.taxprice == null }">readonly</c:if>"  required="required" validType="intOrFloat"  <c:if test="${colMap.taxprice == null }">readonly="readonly"</c:if> style="width:150px;"  tabindex="6" /></td>
                    <td style="text-align: right;">含税金额：</td>
                    <td><input type="text" id="account-goldTaxCustomerInvoiceDetail-taxamount" name="taxamount" class="easyui-validatebox formaterNum <c:if test="${colMap.taxamount == null }">readonly</c:if>"  required="required" validType="intOrFloat"  <c:if test="${colMap.taxamount == null }">readonly="readonly"</c:if> style="width:150px;" tabindex="7" /></td>
                </tr>
                <tr>
                    <td style="text-align: right;">未税单价：</td>
                    <td><input type="text" id="account-goldTaxCustomerInvoiceDetail-notaxprice" name="notaxprice" class="easyui-validatebox formaterNum <c:if test="${colMap.notaxprice == null }">readonly</c:if>"  required="required" validType="intOrFloat" style="width:150px;" tabindex="8" /></td>
                    <td style="text-align: right;">未税金额：</td>
                    <td>
                        <input type="text" id="account-goldTaxCustomerInvoiceDetail-notaxamount" name="notaxamount" class="easyui-validatebox formaterNum <c:if test="${colMap.notaxamount == null }">readonly</c:if>"  required="required" validType="intOrFloat" style="width:150px;" tabindex="9" />
                    </td>
                </tr>
                <tr>
                    <td style="text-align: right;">税率：</td>
                    <td>
                        <input type="text" id="account-goldTaxCustomerInvoiceDetail-taxrate" name="taxrate" class="easyui-validatebox formaterNum <c:if test="${colMap.taxrate == null }">readonly</c:if>" required="required" validType="integer" style="width:150px;" tabindex="10" />
                    </td>
                    <td style="text-align: right;">税额：</td>
                    <td><input type="text" id="account-goldTaxCustomerInvoiceDetail-tax" name="tax" class="easyui-numberbox" data-options="precision:2" readonly="readonly" style="width:150px; "/></td>
                </tr>
                <tr>
            ...
            <input type="hidden" id="account-goldTaxCustomerInvoiceDetail-upid" name="upid" />
            <input type="hidden" id="account-goldTaxCustomerInvoiceDetail-sourcetype" name="sourcetype"/>
        </form>
    </div>
    <div data-options="region:'south',border:false">
        <div class="buttonDetailBG" style="height:30px;text-align:right;">
            <input type="button" value="继续添加(Ctrl+Enter)" name="savegoon" id="account-goldTaxCustomerInvoiceDetailEditPage-editSaveGoOn" style="width: 130px" />&nbsp;<input type="button" value="确定(Alt+M)" name="editsave" id="account-goldTaxCustomerInvoiceDetailEditPage-editSave" />
        </div>
    </div>
</div>
<script type="text/javascript">
    ...
    function orderDetailAddSaveGoOnDialog(){
        ...
        var index=editRowIndex+1;
        isNoData=false;
        if(dataRows.length>index){
            var itemdata=dataRows[index];
            if(itemdata.goodsname!=null && itemdata.goodsname!="") {
                editRowIndex=index;
                orderDetailEditDialog(dataRows[index]);
            }else{
                isNoData=true;
            }
        }else{
        ...
        if(isNoData){
            $.messager.alert("提醒","已经是最后一条记录了。");
        }
    }

    function saveOrderDetail(isGoOn){
        $("#account-goldTaxCustomerInvoiceDetail-remark").focus();
        var flag=$("#account-form-goldTaxCustomerInvoiceDetailEditPage").form('validate');
        if(!flag){
            return false;
        }
        ...
        var formdata=$("#account-form-goldTaxCustomerInvoiceDetailEditPage").serializeJSON();
        //保存前判断单价*数量 是否跟金额一致
    ...
    $(document).ready(function(){
        ...
        $("#account-goldTaxCustomerInvoiceDetail-unitnum").change(function(){
            if($(this).validatebox('isValid')){
                var sourcetype=$("#account-goldTaxCustomerInvoiceDetail-sourcetype").val() || "";
                if(sourcetype=="1"){
        ...
        $("#account-goldTaxCustomerInvoiceDetailEditPage-editSave").click(function(){
            saveOrderDetail(false);
        });
        $("#account-goldTaxCustomerInvoiceDetailEditPage-editSave").die("keydown").live("keydown",function(event){
            if(event.keyCode==13){
                saveOrderDetail(false);
            }
        });
        $("#account-goldTaxCustomerInvoiceDetailEditPage-editSaveGoOn").click(function(){
            saveOrderDetail(true);
        });
        $("#account-goldTaxCustomerInvoiceDetailEditPage-editSaveGoOn").die("keydown").live("keydown",function(event){
            if(event.keyCode==13){
                saveOrderDetail(true);
            }
        });

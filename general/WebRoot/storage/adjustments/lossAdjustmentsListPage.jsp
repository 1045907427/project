...
<div class="easyui-layout" data-options="fit:true">
    <div data-options="region:'north',border:false">
        <div class="buttonBG" id="storage-buttons-lossAdjustmentsPage"></div>
    </div>
    <div data-options="region:'center'">
        <table id="storage-datagrid-lossAdjustmentsPage"></table>
        <div id="storage-datagrid-toolbar-lossAdjustmentsPage" style="padding:2px;height:auto">
            <form action="" id="storage-form-query-lossAdjustmentsPage" method="post">
...
<div id="lossAdjustments-account-dialog"></div>
<script type="text/javascript">
    var initQueryJSON = $("#storage-form-query-lossAdjustmentsPage").serializeJSON();
    $(function () {
        //按钮
        $("#storage-buttons-lossAdjustmentsPage").buttonWidget({
            initButton: [
                {},
                <security:authorize url="/storage/showLossAdjustmentsAddPage.do">
                {
                    type: 'button-add',
                    handler: function () {
                        top.addTab('storage/showLossAdjustmentsAddPage.do', "报损调账单新增");
                    }
                },
                </security:authorize>
                <security:authorize url="/storage/adjustmentsViewPage.do">
                {
                    type: 'button-view',
                    handler: function () {
                        var con = $("#storage-datagrid-lossAdjustmentsPage").datagrid('getSelected');
                        if (con == null) {
                            $.messager.alert("提醒", "请选择一条记录");
                            return false;
                        }
                        top.addOrUpdateTab('storage/showLossAdjustmentsEditPage.do?id=' + con.id, "报损调账单查看");
                    }
                },
                </security:authorize>
                ...
                <security:authorize url="/storage/adjustmentsExport.do">
                {
                    ...
                    attr: {
                        ...
                        queryForm: "#storage-form-query-lossAdjustmentsPage",
            ...
            buttons: [
                <security:authorize url="/erpconnect/addLossAdjustmentsVouch.do">
                {
                    id: 'button-account',
                    name: '生成凭证',
                    iconCls: 'button-audit',
                    handler: function () {
                        var rows = $("#storage-datagrid-lossAdjustmentsPage").datagrid('getChecked');
                        ...
                        $("#lossAdjustments-account-dialog").dialog({
                            title: '报损调账单凭证',
                            width: 400,
                            height: 260,
                            closed: false,
                            modal: true,
                            cache: false,
                            href: 'erpconnect/showLossAdjustmentsVouchPage.do',
                            onLoad: function () {
                                $("#lossAdjustments-ids").val(ids);
                            }
                        });
                    }
                },
        ...
        var checkListJson = $("#storage-datagrid-lossAdjustmentsPage").createGridColumnLoad({
        ...
        $("#storage-datagrid-lossAdjustmentsPage").datagrid({
            ...
            checkOnSelect: true,
            selectOnCheck: true,
            url: 'storage/showLossAdjustmentsList.do',
            queryParams: initQueryJSON,
            toolbar: '#storage-datagrid-toolbar-lossAdjustmentsPage',
            onDblClickRow: function (rowIndex, rowData) {
                top.addOrUpdateTab('storage/showAdjustmentsEditPage.do?billypte=2&id=' + rowData.id, "报损调账单查看");
            }
...
//			});
        ...
        $("#storage-queay-adjustments").click(function () {
            ...
            var queryJSON = $("#storage-form-query-lossAdjustmentsPage").serializeJSON();
            $("#storage-datagrid-lossAdjustmentsPage").datagrid("load", queryJSON);
        });
        //重置
        $("#storage-reload-adjustments").click(function () {
            $("#storage-query-storageid").widget("clear");
            $("#storage-form-query-lossAdjustmentsPage")[0].reset();
            var queryJSON = $("#storage-form-query-lossAdjustmentsPage").serializeJSON();
            $("#storage-datagrid-lossAdjustmentsPage").datagrid("load", queryJSON);
        });
    });
...
<script type="text/javascript">
    $(function () {
        //打印
        AgReportPrint.init({
            id: "lossAdjustmentsPage-dialog-print",
            code: "storage_adjustments",
            tableId: "storage-datagrid-lossAdjustmentsPage",

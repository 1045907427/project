...
<html>
	...
	<body>
		...
		<div id="delivery-layout-edit" class="easyui-layout" data-options="fit:true,border:false">
			<div id="delivery-layout-edit-north" data-options="region:'north',border:false" style="height:160px">
                <form id="delivery-form-head" action="" method="post" style="padding: 3px">
                    <input type="hidden" name="delivery.startpoint" id="storage-startpoint-deliveryEditPage" value="${delivery.startpoint }"/>
					<table cellspacing="3px" cellpadding="3px" border="0">
						<tbody>
							<tr>
								...
								<td width="60px">状&nbsp;&nbsp;态:</td>
								<td><select  style="width: 160px;" disabled="disabled" class="no_input">
									...
									<input type="hidden" id="delivery-delcustomerids-baseinfo" name="delivery.delcustomerids"/>
								</td>
							</tr>
							<tr>
								<td>司机:</td>
								<td><input id="delivery-widget-driverid"  style="width: 160px;" type="text" name="delivery.driverid" value="${delivery.driverid}"/></td>
								<%--<td>--%>
									<%--<table>--%>
										<%--<tr>--%>
									...
									<%--</table>--%>
								<%--</td>--%>
								<td width="60px">线路名称:</td>
								<td><input type="text" style="width: 160px;" name="delivery.linename" value="<c:out value="${delivery.linename}"></c:out>" readonly="readonly" class="no_input" />
                                    <input type="hidden" id="delivery-hidden-lineid" value="${delivery.lineid}"/>
                                </td>
								<td width="60px">车&nbsp;&nbsp;号:</td>
								<td><input type="text" id="delivery-widget-carid" name="delivery.carid" value="<c:out value="${delivery.carid}"></c:out>" /></td>
							</tr>
							<tr>
								<td>跟车:</td>
								<td><input id="delivery-widget-followid" style="width: 160px;" type="text" name="delivery.followid" value="${delivery.followid}"/></td>
								<td width="60px">装车次数:</td>
								<td><input id="delivery-truck-numberbox" type="text" name="delivery.truck" value="${delivery.truck}" style="width: 160px"/></td>
								<td width="60px">送货家数:</td>
							...
							<tr>
								<td>备注:</td>
								<td colspan="5"><input type="text" style="width: 640px;" name="delivery.remark" value="<c:out value="${delivery.remark}"></c:out>" /></td>
							</tr>
						</tbody>
					</table>
				</form>
			...
			<div id="delivery-layout-edit-center" data-options="region:'center',border:false">
				<div class="tagsDiv" style="min-width: 1024px">
					...
					<div class="tagsDiv_item">
						<form id="delivery-form-other" action="" method="post" style="padding: 0px">
							<table cellspacing="5px" cellpadding="5px" border="0">
								<tbody>
									<tr>
										<td width="60px">晚上出车:</td>
										<td style="width: 5px"></td>
										<td><input type="text" style="width: 160px;" name="delivery.nightbonus" data-options="min:0,max:9999999999,precision:2" class="easyui-numberbox" value="${delivery.nightbonus}"<c:if test="${showMap.nightbonus!='nightbonus'}">readonly="readonly"</c:if> /></td>
										<td width="60px">安全奖金:</td>
										<td style="width: 5px"></td>
										<td><input type="text" style="width: 160px;" name="delivery.safebonus" data-options="min:0,max:9999999999,precision:2" class="easyui-numberbox" value="${delivery.safebonus}"<c:if test="${showMap.safebonus!='safebonus'}">readonly="readonly"</c:if> /></td>
										<td width="60px">回单奖金:</td>
										<td style="width: 5px"></td>
										<td><input type="text" style="width: 160px;" name="delivery.receiptbonus" data-options="min:0,max:9999999999,precision:2" class="easyui-numberbox" value="${delivery.receiptbonus}"<c:if test="${showMap.receiptbonus!='receiptbonus'}">readonly="readonly"</c:if> />
										</td>
									</tr>
									<tr>
										<td width="60px">其他补助:</td>
										<td style="width: 5px"></td>
										<td><input type="text" style="width: 160px;" name="delivery.othersubsidy" data-options="min:0,max:9999999999,precision:2" class="easyui-numberbox" value="${delivery.othersubsidy}" <c:if test="${showMap.othersubsidy!='othersubsidy'}">readonly="readonly"</c:if>/></td>
										<td width="60px">送货箱数:</td>
										<td style="width: 5px"></td>
									...
									<tr>
										...
										<td width="60px">装车补助:</td>
										<td style="width: 5px"></td>
										<td>
											<input id="delivery-numberbox-trucksubsidy" type="text" style="width: 160px;" data-options="min:0,max:9999999999,precision:2" class="easyui-numberbox" value="${delivery.trucksubsidy}" disabled="disabled" />
											<input id="delivery-hidden-trucksubsidy" type="hidden" name="delivery.trucksubsidy" value="${delivery.trucksubsidy}"/>
										</td>
									</tr>
								</tbody>
							</table>
					...
					<security:authorize url="/storage/deliveryGoodsTab.do">
						<div class="tagsDiv_item">
							<div id="storage-goods-deliveryEditPage" style="display: none;"></div>
						</div>
					</security:authorize>
					<security:authorize url="/storage/deliveryMapTab.do">
						<div class="tagsDiv_item">
							<div id="storage-dummy-deliveryEditPage" style="display: none;"></div>
						</div>
					</security:authorize>
				</div>
			</div>
		</div>
		<div class="easyui-menu" id="storage-contextMenu-deliveryCustomer" style="display: none;">
	    ...
	    <div class="easyui-menu" id="storage-contextMenu-deliverySaleout" style="display: none;">
        ...
        var $goods = $("#storage-goods-deliveryEditPage");
		...
		$(function(){
   				...
   				onChange:function(newVal,oldVal){
   					...
   					$.ajax({
						...
						success:function(json){
							...
							$("#delivery-hidden-trucksubsidy").val(json.trucksubsidy);
						}
   					});
   				}
			});
			...
			$(".tags").find("li").click(function(){
                var height = $("#delivery-layout-edit-center").height()-5;
					...
					if(!$dgCustomerList.hasClass("create-datagrid")){
						$dgCustomerList.datagrid({
		    				...
		    				onRowContextMenu:function(e, rowIndex, rowData){
			    				...
			    				document.getElementById("storage-removeRow-deliveryCustomer").style.display = "block";
				...
				if(index == 1){
					...
					if(!$dgSaleoutList.hasClass("create-datagrid")){
						$dgSaleoutList.datagrid({
		    				...
		    				onRowContextMenu:function(e, rowIndex, rowData){
			    				e.preventDefault();
			    				$dgSaleoutList.datagrid('selectRow', rowIndex);
			                    $("#storage-contextMenu-deliverySaleout").menu('show', {  
			                        left:e.pageX,  
			                        top:e.pageY  
			                    });
			    			},
				...
				if(index == 4) {
                    ...
                    if(!window.mapframe1) {
                        ...
                        $('#storage-dummy-deliveryEditPage').after('<iframe name="mapframe1" src="storage/deliveryCustomerMapPage.do?type=edit&deliveryid=<c:out value="${delivery.id }"></c:out>" style="width: 100%; height: ' + height + 'px; border: 0px;"></iframe>');
                    }
                }

	    	...
	    	<div data-options="region:'north',border:false" style="height:105px;">
	    		<table style="border-collapse:collapse;" border="0" cellpadding="5px" cellspacing="5px">
	    			<tr>
	    				...
	    				<td class="len165"><input class="easyui-validatebox" style="width: 136px" name="lend.id" <c:if test="${autoCreate == true }">readonly='readonly'</c:if> <c:if test="${autoCreate == false }">required="required"</c:if> /></td>
	    				<td class="len80 left">业务日期：</td>
	    				<td class="len165"><input type="text" class="len130" style="width: 136px" onfocus="WdatePicker({'dateFmt':'yyyy-MM-dd'})" value="${busdate }" name="lend.businessdate" /></td>
    	...
    	$(function(){
    		...
    		$("#storage-form-lendAdd").form({
			    onSubmit: function(){  
					...
					$("#storage-lendOut-lendDetail").val(JSON.stringify(json));
			    ...
			    success:function(data){
			    	...
			    	if(json.flag){
			    		...
			    		$("#storage-panel-lendPage").panel({
							href:"storage/lendEditPage.do?billtype=1&id="+json.id,
						    cache:false,
						    maximized:true,
						    border:false
						});

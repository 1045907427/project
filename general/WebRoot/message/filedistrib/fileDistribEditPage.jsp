...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
  	  	<div style="height:100%;margin:10px 20px; ">
				<form action="message/filedistrib/editFileDistrib.do" method="post" id="fileDistrib-form-fileDistribAddPage" opertype="edit">
					...
					<div style="hiehgt:30px;">
						...
						<select id="fileDistrib-form-fileDistribAddPage-type"  name="fileDistrib.type">
					...
					<div style="hiehgt:30px;padding-bottom:4px;">
						...
						<div style="float:left;height:16px;padding-top:3px;">
							<input type="text" id="fileDistrib-form-fileDistribAddPage-title" name="fileDistrib.title" value="${fileDistrib.title }" class="easyui-validatebox" required="true" style="width:350px;"/>
						</div>
						<div style="clear:both"></div>
					</div>
					<div style="hiehgt:30px;padding-bottom:4px;">
						...
						<div style="float:left;height:16px;padding-top:3px;">
							<input type="text" id="fileDistrib-form-fileDistribAddPage-titleword" name="fileDistrib.titleword" value="${fileDistrib.titleword }" class="easyui-validatebox" required="true" style="width:350px;"/>
						</div>
						<div style="clear:both"></div>
					</div>
					<div style="hiehgt:30px;padding-bottom:4px;">
						...
						<div style="float:left;height:16px;padding-top:3px;">
							<input type="text" id="fileDistrib-form-fileDistribAddPage-wordsize" name="fileDistrib.wordsize" value="${fileDistrib.wordsize }" class="easyui-validatebox" required="true" style="width:350px;"/>
						</div>
						<div style="clear:both"></div>
					</div>
					...
					<div style="height:35px;line-height:30px;">
						...
						<div style="float:left">
							<input type="text" style="width:80px" name="fileDistrib.startdate" value="${fileDistrib.startdate }" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly"/>至<input type="text" style="width:80px" name="fileDistrib.enddate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly"/>&nbsp;为空为手动终止
						</div>
						<div style="clear:both"></div>
					</div>					
					<div style="height:30px;">
						...
						<div style="float:left">
		    				<input type="checkbox" id="fileDistrib-form-fileDistribAddPage-ismsg" name="fileDistrib.ismsg" value="1" <c:if test="${fileDistrib.ismsg==1 }" > checked="checked" </c:if>/>使用内部短信提醒 &nbsp;
		    			</div>
						<div style="clear:both"></div>
					</div>
					<div style="height:30px;">
						...
						<div style="float:left">
		    				<input type="checkbox" id="fileDistrib-form-fileDistribAddPage-istop" name="fileDistrib.istop" value="1" <c:if test="${fileDistrib.istop==1 }" > checked="checked" </c:if> />置顶，显示为重要&nbsp;
					...
					<div id="fileDistrib-form-fileDistribAddPage-cfile-flash" style="width:780px;height:450px;  display:none;">
					 ...
					 --%>
					<div style="clear:both"></div>
					<input type="hidden" name="fileDistrib.id" value="${fileDistrib.id }"/>
				</form>
				<br/>
				<div style="clear:both"></div>
		</div>			
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
		  		...
		  		$("#fileDistrib-buttons-fileDistribPage").buttonWidget("setDataID",  {id:'${fileDistrib.id}',state:'${fileDistrib.state}',type:'edit'});
		  		initLoadForm();
	    		...
	    		//角色 
	    	  	$("#fileDistrib-form-fileDistribAddPage-recvrolename").widget({
	    			...
	    			onLoadSuccess:function(){
	    				...
	    				if(temp.toUpperCase()=="ALL"){
		    				$("#fileDistrib-form-fileDistribAddPage-btn-addRole").click();
	    				}
	    			}
	    		}); 
					...
					onComplete: function(data){
				    	...
				    	for(var i=0;i<data.length;i++){
				    		...
				    		filearr.push(data[i].id)
						   	...
						   	htmlsb.push("<div style=\"float:left;display:block;margin-right:10px;\" ");
						   	htmlsb.push(" onmouseover =\"javascript:showAttachMenu(this,'"+data[i].id+"')\" ");
							htmlsb.push(" fileid=\"");
							htmlsb.push(data[i].id);
							htmlsb.push("\">");
						   	htmlsb.push("<a href='common/download.do?id="+data[i].id+"' target='_blank' title='点击下载'>");
						   	htmlsb.push(data[i].oldFileName);
						   	htmlsb.push("</a>");
						   	htmlsb.push(showAttachMenuContent(data[i],false));
						   	htmlsb.push("</div>");
						   	$("#fileDistrib-form-fileDistribAddPage-attachment-uplist").append(htmlsb.join(""));
				    	}

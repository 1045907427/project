...
public class ActivityImpl extends ScopeImpl implements PvmActivity, HasDIBounds {
   ...
   * @return
   */
  public TransitionImpl createCustomOutgoingTransition(String transitionId, String taskKey ) {
          TransitionImpl transition = new TransitionImpl(transitionId, processDefinition);
            transition.setSource(this);
            ActivityImpl destinationTask = processDefinition.findActivity(taskKey);
            if(destinationTask == null)
                    throw new PvmException("Create Custom OutgoingTransition for activity '"+id+"' with transitionId '"+transitionId+"' and destinationTaskKey '"+taskKey+"' failed: activity[" + taskKey +"] not found in process[" + processDefinition.getId() +"]");

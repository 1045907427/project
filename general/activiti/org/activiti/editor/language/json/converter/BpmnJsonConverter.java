	...
	protected static final Logger LOGGER = Logger.getLogger(BpmnJsonConverter.class.getName());
	  ...
	  public BpmnModel convertToBpmnModel(JsonNode modelNode) {
	    ...
	    if(processDocumentationNode != null && StringUtils.isNotEmpty(processDocumentationNode.asText())){
	    	bpmnModel.getMainProcess().setDocumentation(processDocumentationNode.asText());
	    }
	    
	    //JsonNode listener = modelNode.get(EDITOR_SHAPE_PROPERTIES).get(PROPERTY_EXECUTION_LISTENERS);
	    //JsonNode t1 = 
	    try{
	    	JsonNode listenersNode = modelNode.get(EDITOR_SHAPE_PROPERTIES).get(PROPERTY_EXECUTION_LISTENERS);
	    	      ...
	    	      if (itemsArrayNode != null) {
	    	        for (JsonNode itemNode : itemsArrayNode) {
	    	          ...
	    	          if (typeNode != null && StringUtils.isNotEmpty(typeNode.asText())) {
	    	            ...
	    	            if (StringUtils.isNotEmpty(getValueAsString(PROPERTY_EXECUTION_LISTENER_CLASS, itemNode))) {
	    	              listener.setImplementationType(ImplementationType.IMPLEMENTATION_TYPE_CLASS);
	    	              listener.setImplementation(getValueAsString(PROPERTY_EXECUTION_LISTENER_CLASS, itemNode));
	    	            } else if (StringUtils.isNotEmpty(getValueAsString(PROPERTY_EXECUTION_LISTENER_EXPRESSION, itemNode))) {
	    	              listener.setImplementationType(ImplementationType.IMPLEMENTATION_TYPE_EXPRESSION);
	    	              listener.setImplementation(getValueAsString(PROPERTY_EXECUTION_LISTENER_EXPRESSION, itemNode));
	    	            } else if (StringUtils.isNotEmpty(getValueAsString(PROPERTY_EXECUTION_LISTENER_DELEGATEEXPRESSION, itemNode))) {
	    	              listener.setImplementationType(ImplementationType.IMPLEMENTATION_TYPE_DELEGATEEXPRESSION);
	    	              listener.setImplementation(getValueAsString(PROPERTY_EXECUTION_LISTENER_DELEGATEEXPRESSION, itemNode));
	    	            }
	    	            bpmnModel.getMainProcess().getExecutionListeners().add(listener);
	    	          }
	    	        }
	    	      }
	    	    }

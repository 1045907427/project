...
public class JRXhtmlExporter extends AbstractHtmlExporter<HtmlReportConfiguration, HtmlExporterConfiguration>
{
	...
	protected void exportReportToWriter() throws JRException, IOException
	{
		...
		/**修改开始**/
		String wrapBeforePageHtml = (String)parameters.get(JRHtmlExporterParameter.WRAP_BEFORE_PAGE_HTML);
		String wrapAfterPageHtml = (String)parameters.get(JRHtmlExporterParameter.WRAP_AFTER_PAGE_HTML);
		/**修改结束**/
		...
		if (htmlHeader == null)
		{
			...
			writer.write("    * {padding:0;margin:0;}\n");
			writer.write("  </style>\n");
			writer.write("</head>\n");
		...
		for(reportIndex = 0; reportIndex < items.size(); reportIndex++)
		{
			...
			if (pages != null && pages.size() > 0)
			{
				...
				for(pageIndex = startPageIndex; pageIndex <= endPageIndex; pageIndex++)
				{
					...
					if(null!=wrapBeforePageHtml && !"".equals(wrapBeforePageHtml)){
						writer.write(wrapBeforePageHtml);
						if(jasperPrint!=null) {
							String agprintParamsDatahtml = jasperPrint.getProperty("agprint_params_datahtml");
							if (null != agprintParamsDatahtml && !"".equals(agprintParamsDatahtml.trim())) {
								agprintParamsDatahtml= Base64.encodeBase64String(agprintParamsDatahtml.getBytes("UTF-8"));
								writer.write("<input type=\"hidden\" myattr=\"agprintparamsdatahtml\" jsondata=\"" + agprintParamsDatahtml + "\"/>");
							}
						}

						writer.write("<input type=\"hidden\" myattr=\"showprintpage\" current=\""+(startPageIndex+1)+"\" total=\""+(endPageIndex+1)+"\"/>");
					}
					
					exportPage(page);

					if(null!=wrapAfterPageHtml && !"".equals(wrapAfterPageHtml)){
						writer.write(wrapAfterPageHtml);
					}
				...
				else
				{
					...
					else
					{
						...
						if (imageHandler != null)
						{
							...
							Boolean isOutputImagesToDBParameter = (Boolean)parameters.get(JRHtmlExporterParameter.IS_OUTPUT_IMAGES_TO_DB);
							if(isOutputImagesToDBParameter!=null && isOutputImagesToDBParameter) {
								String imagesUri = (String)parameters.get(JRHtmlExporterParameter.IMAGES_URI);
								if(null!=imagesUri && !"".equals(imagesUri)){
									String agprintParamsDatahtml = jasperPrint.getProperty("agprint_params_datahtml");
									imageHandler = new DBFileHtmlResourceHandler(  imagesUri,agprintParamsDatahtml );
									if(imageHandler!=null){

...
public class RejectbillPrintAction extends BaseFilesAction {
    ...
    private String rejectbillPrintHandle(boolean isview) throws Exception {
        ...
        ajaxResultMap.put("printname", "客户退货订单打印-" + CommonUtils.getDataNumberSeconds());
        ...
        try {
            ...
            if(isview){
                ...
                ajaxResultMap.put("printname", "客户退货订单打印任务-" + printJob.getId());
            }
            String printlimit = getPrintLimitInfo();
            Map map = new HashMap();
            map.put("idarrs", idarrs);
            map.put("showdetail", "1");
            List<DeliveryRejectbill> list = distributeService.printRejectbill(map);
            if (list.size() == 0) {
                if ("ajaxhtml".equals(viewtype)) {
                    ajaxResultMap.put("flag", false);
                    ajaxResultMap.put("msg", "未能找到相关打印单据");
            ...
            for (DeliveryRejectbill item : list) {
                ...
                List<DeliveryRejectbillDetail> detailList = item.getDeliveryRejectbillDetail();


                templetQueryMap.clear();
                templetQueryMap.put("realServerPath", servletContext.getRealPath("/"));
                templetQueryMap.put("code", "delivery_rejectbill");
                ...
                //String reportModelFile = servletContext.getRealPath("/ireport/purchase_buyrejectbill/purchase_buyrejectbill.jasper");
                jreport = (JasperReport) JRLoader.loadObject(new File(printTempletFile));
                parameters = new HashMap();
                //parameters.put("P_BuyRejectbill", rejectbillInfo);
                ...
                /**打印模板参数用的单据信息**/
                DeliveryRejectbill orderInfo = (DeliveryRejectbill) CommonUtils.deepCopy(item);
                orderInfo.setDeliveryRejectbillDetail(null);
                ...
                if (null != jrprint) {
                    jrprint.setName("客户退货订单-" + item.getId());
                    jrlist.add(jrprint);
                    if (!isview) {
                        ...
                        if (StringUtils.isNotEmpty(printJob.getId())) {
                            ...
                            printJobCallHandle.setPrintordername("客户退货订单打印");
                            printJobCallHandle.setSourceorderid(item.getId());
                            printJobCallHandle.setSourceordername("客户退货订单打印");
            ...
            if (!isview) {
                ...
                //printLogsb.append(" 。");
                printLogsb.append(" 客户退货订单编号： " + canprintIds);
                printLogsb.append(" 。");
                printLogsb.append(" 操作名称： 客户退货订单打印。");
            ...
            StringBuilder printLogsb = new StringBuilder();
            if (!isview) {
                printLogsb.append("打印申请-客户退货订单打印异常：");
                //printLogsb.append("申请打印的采购订单编号："+idarrs);
                //printLogsb.append(" 。");
                printLogsb.append("客户退货订单编号： " + canprintIds);
                printLogsb.append(" 。");
            } else {
                printLogsb.append("打印预览申请-客户退货订单打印预览异常：");
            }
            printLogsb.append("Exception ：rejectbillPrint()>>>>>>>>>>>>>>>>异常信息： " + ex.getMessage());
            addPrintLogInfo("PrintHandle-rejectbill", printLogsb.toString(), null);
            logger.error("客户退货订单打印及预览处理异常", ex);
    ...
    public void rejectbillPrintCallBackHandle(String printcallback) throws Exception {
        ...
        if (null != id && !"".equals(id.trim())) {
            ...
            printLogsb.append("客户退货订单编号： " + id);
            try {
                Map deliveryRejectbillMap = distributeService.getDeliveryRejectbillInfo(id);
                DeliveryRejectbill deliveryRejectbill = (DeliveryRejectbill) deliveryRejectbillMap.get("deliveryRejectbill");
                if (null != deliveryRejectbill) {

                    deliveryRejectbill.setPrinttimes(deliveryRejectbill.getPrinttimes() + 1);
                    distributeService.updateRejectbillPrinttimes(deliveryRejectbill);
                ...
                logger.error("客户退货订单更新打印次数异常", ex);
            }
        } else {
            printLogsb.append(" 。");
        ...
        printLogsb.append(" 打印数据来源(客户退货订单编号)：");

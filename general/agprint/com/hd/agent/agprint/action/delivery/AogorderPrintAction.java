 ...
 *
 */
public class AogorderPrintAction extends BaseFilesAction {
    //region 参数初始化
    private static final Logger logger = Logger.getLogger(AogorderPrintAction.class);
     ...
     * @date 2015-9-2
     */
    public String aogorderPrintView() throws Exception {
        return aogorderPrintHandle(true);
    }
     ...
     * @date 2015-9-2
     */
    public String aogorderPrint() throws Exception {
        ...
        if (null != printcallback && !"".equals(printcallback)) {
            aogorderPrintCallBackHandle(printcallback);
        } else {
            ...
            return aogorderPrintHandle(isview);
        }
        return null;
    }
     ...
     * @date 2015-9-2
     */
    private String aogorderPrintHandle(boolean isview) throws Exception {
        ...
        try {
            ...
            List<DeliveryAogorder> list = distributeService.printAogorder(map);
            if (list.size() == 0) {
                if ("ajaxhtml".equals(viewtype)) {
                    ajaxResultMap.put("flag", false);
                    ajaxResultMap.put("msg", "未能找到相关打印单据");
            ...
            for (DeliveryAogorder item : list) {
                ...
                List<DeliveryAogorderDetail> detailList = item.getDeliveryAogorderDetail();
                ...
                templetQueryMap.put("code", "delivery_aogorder");
                ...
                /**打印模板参数用的单据信息**/
                DeliveryAogorder orderInfo = (DeliveryAogorder) CommonUtils.deepCopy(item);
                orderInfo.setDeliveryAogorderDetail(null);
                ...
                if (null != jrprint) {
                    jrprint.setName("到货订单-" + item.getId());
                    jrlist.add(jrprint);
                    if (!isview) {
                        ...
                        if (StringUtils.isNotEmpty(printJob.getId())) {
                            ...
                            printJobCallHandle.setClassname("aogorderPrintAction");
                            printJobCallHandle.setMethodname("aogorderPrintReflectCallBackHandle");
                            ...
                            printJobCallHandle.setPrintordername("到货订单打印");
                            printJobCallHandle.setSourceorderid(item.getId());
                            printJobCallHandle.setSourceordername("到货订单打印");
            ...
            if (!isview) {
                ...
                //printLogsb.append(" 。");
                printLogsb.append(" 到货订单编号： " + canprintIds);
                printLogsb.append(" 。");
                printLogsb.append(" 操作名称： 到货订单打印。");
                addPrintLogInfo("PrintHandle-Aogorder", printLogsb.toString(), null);
            }
            //输出打印报表
            if ("ajaxhtml".equals(viewtype)) {
            ...
            StringBuilder printLogsb = new StringBuilder();
            if (!isview) {
                printLogsb.append("打印申请-到货订单打印：");
                //printLogsb.append("申请打印的采购订单编号："+idarrs);
                //printLogsb.append(" 。");
                printLogsb.append("到货订单编号： " + canprintIds);
                printLogsb.append(" 。");
            } else {
                printLogsb.append("打印预览申请-到货订单打印预览：");
            }
            printLogsb.append("Exception ：aogorderPrint()>>>>>>>>>>>>>>>>异常信息： " + ex.getMessage());
            addPrintLogInfo("PrintHandle-aogorder", printLogsb.toString(), null);
            logger.error("到货订单打印及预览处理异常", ex);
     ...
     * @date 2015-9-2
     */
    public void aogorderPrintCallBackHandle(String printcallback) throws Exception {
        String idarrs = request.getParameter("idarrs");
        if (null == idarrs) {
            idarrs = "";
        }
        ...
        if (null == printcallback || "".equals(printcallback)) {
            ...
            printLogsb.append(" 。");
            addPrintLogInfo("PrintCallBackHandle-aogorder", printLogsb.toString(), null);
            response.reset();
        ...
        if (null != id && !"".equals(id.trim())) {
            ...
            printLogsb.append("到货订单编号： " + id);
            try {
                Map deliveryAogorderMap = distributeService.getDeliveryAogorderInfo(id);
                DeliveryAogorder deliveryAogorder = (DeliveryAogorder) deliveryAogorderMap.get("deliveryAogorder");
                if (null != deliveryAogorder) {

                    deliveryAogorder.setPrinttimes(deliveryAogorder.getPrinttimes() + 1);
                    distributeService.updateAogorderPrinttimes(deliveryAogorder);
                ...
                logger.error("到货订单打印更新打印次数异常", ex);
            }
        } else {
            printLogsb.append(" 。");
        ...
        printLogsb.append(" 打印数据来源(到货订单编号)：");
     ...
     * @date Nov 08, 2016
     */
    public boolean aogorderPrintReflectCallBackHandle(String printcallback) throws Exception {
        boolean isok = false;
        StringBuilder printLogsb = new StringBuilder();
        //printLogsb.append("更新打印次数：");
        if (null == printcallback || "".equals(printcallback)) {

 ...
 * @date 2015-01-14
 */
public class StorageOtherEnterPrintAction extends BaseFilesAction {
    //region 参数初始化
    private static final Logger logger = Logger.getLogger(StorageOtherEnterPrintAction.class);
     ...
     * @date 2015-01-14
     */
    public String storageOtherEnterPrintView() throws Exception {
        return storageOtherEnterPrintHandle(true);
    }
     ...
     * @date 2015-01-14
     */
    @UserOperateLog(key = "StorageOtherEnter-Storage", type = 0)
    public String storageOtherEnterPrint() throws Exception {
        ...
        if (null != printcallback && !"".equals(printcallback)) {
            storageOtherEnterPrintCallBackHandle(printcallback);
        } else {
            ...
            return storageOtherEnterPrintHandle(isview);
        }
        return null;
    }
     ...
     * @date 2015-01-14
     */
    private String storageOtherEnterPrintHandle(boolean isview) throws Exception {
        ...
        ajaxResultMap.put("printname", "其他入库单打印-" + CommonUtils.getDataNumberSeconds());
        ...
        try {
            ...
            if(isview){
                ...
                ajaxResultMap.put("printname", "其他入库单打印任务-" + printJob.getId());
            }
            ...
            List<StorageOtherEnter> list = storageOtherEnterService.showStorageOtherEnterListBy(map);
            if (list.size() == 0) {
                if ("ajaxhtml".equals(viewtype)) {
                    ajaxResultMap.put("flag", false);
                    ajaxResultMap.put("msg", "未能找到相关打印单据");
            ...
            for (StorageOtherEnter item : list) {
                List<StorageOtherEnterDetail> detailList = item.getOtherEnterDetailList();
                ...
                templetQueryMap.put("code", "storage_otherenter");
                ...
                /**打印模板参数用的单据信息**/
                StorageOtherEnter orderInfo = (StorageOtherEnter) CommonUtils.deepCopy(item);
                orderInfo.setOtherEnterDetailList(null);
                ...
                if (null != jrprint) {
                    jrprint.setName("其他入库-" + item.getId());
                    jrlist.add(jrprint);
                    ...
                    if (!isview) {
                        ...
                        if (StringUtils.isNotEmpty(printJob.getId())) {
                            ...
                            printJobCallHandle.setClassname("storageOtherEnterPrintAction");
                            printJobCallHandle.setMethodname("storageOtherEnterPrintReflectCallBackHandle");
                            ...
                            printJobCallHandle.setPrintordername("其他入库单打印");
                            printJobCallHandle.setSourceorderid(item.getId());
                            printJobCallHandle.setSourceordername("其他入库单打印");
            ...
            if (!isview) {
                ...
                //printLogsb.append("申请打印的其他入库编号："+idarrs);
                //printLogsb.append(" 。");
                printLogsb.append(" 其他入库编号： " + canprintIds);
                printLogsb.append(" 。");
                printLogsb.append(" 操作名称： 其他入库打印。");
                addPrintLogInfo("PrintHandle-storageOtherEnter", printLogsb.toString(), null);
            }
            ...
            StringBuilder printLogsb = new StringBuilder();
            if (!isview) {
                printLogsb.append("打印申请-其他入库打印异常：");
                //printLogsb.append("申请打印的其他入库编号："+idarrs);
                //printLogsb.append(" 。");
                printLogsb.append("其他入库编号： " + canprintIds);
                printLogsb.append(" 。");
            } else {
                printLogsb.append("打印预览申请-其他入库打印预览异常：");
            }
            printLogsb.append("Exception ：storageOtherEnterPrint()>>>>>>>>>>>>>>>>异常信息: " + ex.getMessage());
            addPrintLogInfo("PrintHandle-storageOtherEnter", printLogsb.toString(), null);
            logger.error("其他入库单打印及预览处理异常", ex);
     ...
     * @date 2014-1-6
     */
    public void storageOtherEnterPrintCallBackHandle(String printcallback) throws Exception {
        String idarrs = request.getParameter("idarrs");
        if (null == idarrs) {
            idarrs = "";
        }
        ...
        if (null == printcallback || "".equals(printcallback)) {
            ...
            printLogsb.append(" 。");
            addPrintLogInfo("PrintCallBackHandle-storageOtherEnter", printLogsb.toString(), null);
            response.reset();
        ...
        if (null != id && !"".equals(id.trim())) {
            //printLogsb.append("申请打印的其他入库编号："+idarrs);
            //printLogsb.append(" 。");
            printLogsb.append("其他入库编号： " + id);
            try {
                StorageOtherEnter storageOtherEnter = storageOtherEnterService.showPureStorageOtherEnter(id);
                if (null != storageOtherEnter) {
                    storageOtherEnter.setPrinttimes(1);
                    storageOtherEnterService.updateOrderPrinttimes(storageOtherEnter);
                ...
                logger.error("其他入库单更新打印次数异常", ex);
            }
        } else {
            printLogsb.append(" 。");
            printLogsb.append("回调参数中无其他入库编号信息");
        }
        printLogsb.append(" 。");
		/*
		...
		*/
        //printLogsb.append(" 更新响应来源于： 其他入库打印。");
        printLogsb.append(" 打印数据来源(其他入库编号)：");
     ...
     * @date Nov 14, 2016
     */
    public boolean storageOtherEnterPrintReflectCallBackHandle(String printcallback) throws Exception {
        boolean isok = false;
        StringBuilder printLogsb = new StringBuilder();
        //printLogsb.append("更新打印次数：");
        if (null == printcallback || "".equals(printcallback)) {
        ...
        if (null != id && !"".equals(id.trim())) {
            ...
            try {
                ...
                if (null != storageOtherEnter) {
                    storageOtherEnter.setPrinttimes(1);
                    isok = storageOtherEnterService.updateOrderPrinttimes(storageOtherEnter);
                    printLogsb.append(" 更新打印次数成功");
                } else {
                    printLogsb.append(" 更新打印次数失败");
                }

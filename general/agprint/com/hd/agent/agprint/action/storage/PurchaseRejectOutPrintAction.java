/**
 * @(#)PurchaseRejectOutPrintAction.java
 * @author zhanghonghui
 * <p>
 * 版本历史
 * -------------------------------------------------------------------------
 ...
 * @author zhanghonghui
 */
public class PurchaseRejectOutPrintAction extends BaseFilesAction {
    //region 参数初始化
    private static final Logger logger = Logger.getLogger(PurchaseRejectOutPrintAction.class);
     ...
     * @date 2013-11-12
     */
    public String purchaseRejectOutPrintView() throws Exception {
        return purchaseRejectOutPrintHandle(true);
    }
    ...
    public String purchaseRejectOutPrint() throws Exception {
        ...
        if (null != printcallback && !"".equals(printcallback)) {
            purchaseRejectOutPrintCallBackHandle(printcallback);
        } else {
            ...
            return purchaseRejectOutPrintHandle(isview);
        }
        return null;
    }
     ...
     * @date 2013-11-12
     */
    private String purchaseRejectOutPrintHandle(boolean isview) throws Exception {
        ...
        ajaxResultMap.put("printname", "采购退货出库单打印-" + CommonUtils.getDataNumberSeconds());
        ...
        try {
            ...
            if(isview){
                ...
                ajaxResultMap.put("printname", "采购退货出库单打印任务-" + printJob.getId());
            }
            Map map = new HashMap();
            map.put("idarrs", idarrs);
            map.put("statusarr", "3,4");
            map.put("showdetail", "1");
            List<PurchaseRejectOut> list = purchaseRejectOutService.getPurchaseRejectOutListBy(map);
            if (list.size() == 0) {
                if ("ajaxhtml".equals(viewtype)) {
                    ajaxResultMap.put("flag", false);
                    ajaxResultMap.put("msg", "未能找到相关打印单据");
            ...
            for (PurchaseRejectOut item : list) {
                List<PurchaseRejectOutDetail> detailList = item.getBillDetailList();
                ...
                templetQueryMap.put("code", "storage_purchasereject");
                ...
                //String reportModelFile = servletContext.getRealPath("/ireport/storage_purchasereject/storage_purchasereject.jasper");
                ...
                /**打印模板参数用的单据信息**/
                PurchaseRejectOut orderInfo = (PurchaseRejectOut) CommonUtils.deepCopy(item);
                ...
                if (null != jrprint) {
                    jrprint.setName("采购退货-" + item.getSourceid());
                    jrlist.add(jrprint);
                    ...
                    if (!isview) {
                        ...
                        if (StringUtils.isNotEmpty(printJob.getId())) {
                            ...
                            printJobCallHandle.setClassname("purchaseRejectOutPrintAction");
                            printJobCallHandle.setMethodname("purchaseRejectOutPrintReflectCallBackHandle");
                            ...
                            printJobCallHandle.setPrintordername("采购退货出库单打印");
                            printJobCallHandle.setSourceorderid(item.getId());
                            printJobCallHandle.setSourceordername("采购退货出库单打印");
            ...
            if (!isview) {
                ...
                //printLogsb.append("申请打印的采购退货出库单编号："+idarrs);
                //printLogsb.append(" 。");
                printLogsb.append("采购退货出库单编号： " + canprintIds);
                printLogsb.append(" 。");
                printLogsb.append(" 操作名称：采购退货出库单打印 。");
                addPrintLogInfo("PrintHandle-purchaseRejectOut", printLogsb.toString(), null);
            }
            ...
            StringBuilder printLogsb = new StringBuilder();
            if (!isview) {
                printLogsb.append("打印申请-采购退货出库单打印异常：");
                //printLogsb.append("申请打印的采购退货出库单编号："+idarrs);
                //printLogsb.append(" 。");
                printLogsb.append("采购退货出库单编号： " + canprintIds);
                printLogsb.append(" 。");
            } else {
                printLogsb.append("打印预览申请-采购退货出库单打印预览异常：");
            }
            printLogsb.append("Exception ：purchaseRejectOutPrintHandle()>>>>>>>>>>>>>>>>异常信息： " + ex.getMessage());
            addPrintLogInfo("PrintHandle-purchaseRejectOut", printLogsb.toString(), null);
            logger.error("采购退货出库单打印及预览处理异常", ex);
     ...
     * @date 2014-1-6
     */
    public void purchaseRejectOutPrintCallBackHandle(String printcallback) throws Exception {
        String idarrs = request.getParameter("idarrs");
        if (null == idarrs) {
            idarrs = "";
        }
        ...
        if (null == printcallback || "".equals(printcallback)) {
            ...
            printLogsb.append(" 。");
            addPrintLogInfo("PrintCallHandle-purchaseRejectOut", printLogsb.toString(), null);
            response.reset();
        ...
        if (null != id && !"".equals(id.trim())) {
            //printLogsb.append("申请打印的采购退货出库单编号："+idarrs);
            //printLogsb.append(" 。");
            printLogsb.append("采购退货出库单编号： " + id);
            try {
                PurchaseRejectOut purchaseRejectOut = purchaseRejectOutService.getPurchaseRejectOutPureInfo(id);
                if (null != purchaseRejectOut) {
                    purchaseRejectOut.setPrinttimes(1);
                    purchaseRejectOutService.updateOrderPrinttimes(purchaseRejectOut);
                ...
                logger.error("采购退货出库单更新打印次数异常", ex);
            }
        } else {
            printLogsb.append(" 。");
            printLogsb.append("回调参数中无采购退货出库单编号信息");
        }
        printLogsb.append(" 。");
        printLogsb.append(" 打印数据来源(采购退货出库单编号)：");
     ...
     * @date 2014-1-6
     */
    public boolean purchaseRejectOutPrintReflectCallBackHandle(String printcallback) throws Exception {
        boolean isok = false;
        StringBuilder printLogsb = new StringBuilder();
        //printLogsb.append("更新打印次数：");
        if (null == printcallback || "".equals(printcallback)) {
        ...
        if (null != id && !"".equals(id.trim())) {
            ...
            try {
                ...
                if (null != purchaseRejectOut) {
                    purchaseRejectOut.setPrinttimes(1);
                    isok = purchaseRejectOutService.updateOrderPrinttimes(purchaseRejectOut);
                    printLogsb.append(" 更新打印次数成功");
                } else {
                    printLogsb.append(" 更新打印次数失败");
                }

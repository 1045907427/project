 ...
 * @date 2015-01-14
 */
public class StorageLendPrintAction extends BaseFilesAction {
    //region 参数初始化
    private static final Logger logger = Logger.getLogger(StorageLendPrintAction.class);
     ...
     * @date 2015-01-14
     */
    public String lendPrintView() throws Exception {
        String billtype = request.getParameter("billtype");
        return lendPrintHandle(true,billtype);
    }
     ...
     * @date 2015-01-14
     */
    @UserOperateLog(key = "Lend-Storage", type = 0)
    public String lendPrint() throws Exception {
        ...
        if (null != printcallback && !"".equals(printcallback)) {
            lendPrintCallBackHandle(printcallback,billtype);
        } else {
            ...
            return lendPrintHandle(isview,billtype);
        }
        return null;
    }
     ...
     * @date 2015-01-14
     */
    private String lendPrintHandle(boolean isview,String billtype) throws Exception {
        ...
        if ("2".equals(billtype)){
            ajaxResultMap.put("printname", "还货单打印-" + CommonUtils.getDataNumberSeconds());
        } else {
            ajaxResultMap.put("printname", "借货单打印-" + CommonUtils.getDataNumberSeconds());
        }
        ...
        try {
            ...
            if ("ajaxhtml".equals(viewtype)) {
                ...
                if ("2".equals(billtype)) {
                    printJob.setJobname("还货单打印");
                }else {
                    printJob.setJobname("借货单打印");
                }
            ...
            if(isview){
                ...
                if ("2".equals(billtype)) {
                    ajaxResultMap.put("printname", "还货单打印任务-" + printJob.getId());
                } else {
                    ajaxResultMap.put("printname", "借货单打印任务-" + printJob.getId());
                }
            }
            ...
            List<Lend> list = lendService.showLendListBy(map);
            if (list.size() == 0) {
                if ("ajaxhtml".equals(viewtype)) {
                    ajaxResultMap.put("flag", false);
                    ajaxResultMap.put("msg", "未能找到相关打印单据");
            ...
            for (Lend item : list) {
                List<LendDetail> detailList = item.getLendDetailList();
                ...
                if ("2".equals(billtype)) {
                    templetQueryMap.put("code", "storage_lendenter");
                }else {
                    templetQueryMap.put("code", "storage_lendout");
                }
                ...
                if ("1".equals(item.getLendtype())) {
                    item.setLendtypename("供应商");
                } else {
                    item.setLendtypename("客户");
                }
                ...
                /**打印模板参数用的单据信息**/
                Lend orderInfo = (Lend) CommonUtils.deepCopy(item);
                orderInfo.setLendDetailList(null);
                ...
                if (null != jrprint) {
                    if ("2".equals(billtype)) {
                        jrprint.setName("还货-" + item.getId());
                    } else {
                        jrprint.setName("借货-" + item.getId());
                    }
                    jrlist.add(jrprint);
                    ...
                    if (!isview) {
                        ...
                        callbackparamMap.put("billtype",billtype);
                        ...
                        if (StringUtils.isNotEmpty(printJob.getId())) {
                            ...
                            printJobCallHandle.setClassname("storageLendPrintAction");
                            printJobCallHandle.setMethodname("lendPrintReflectCallBackHandle");
                            ...
                            if ("2".equals(billtype)) {
                                printJobCallHandle.setPrintordername("还货单打印");
                            } else {
                                printJobCallHandle.setPrintordername("借货单打印");
                            }
                            printJobCallHandle.setSourceorderid(item.getId());
                            if ("2".equals(billtype)) {
                                printJobCallHandle.setSourceordername("还货单打印");
                            } else {
                                printJobCallHandle.setSourceordername("借货单打印");
                            }
            ...
            if (!isview) {
                ...
                //printLogsb.append("申请打印的借货还货编号："+idarrs);
                //printLogsb.append(" 。");
                if ("2".equals(billtype)) {
                    printLogsb.append(" 还货编号： " + canprintIds);
                } else {
                    printLogsb.append(" 借货编号： " + canprintIds);
                }
                printLogsb.append(" 。");
                if ("2".equals(billtype)) {
                    printLogsb.append(" 操作名称： 还货打印。");
                } else {
                    printLogsb.append(" 操作名称： 借货打印。");
                }

                addPrintLogInfo("PrintHandle-lend", printLogsb.toString(), null);
            }
            ...
            if (!isview) {
                if ("2".equals(billtype)) {
                    printLogsb.append("打印申请-还货打印异常：");
                    //printLogsb.append("申请打印的借货还货编号："+idarrs);
                    //printLogsb.append(" 。");
                    printLogsb.append("还货编号： " + canprintIds);
                } else {
                    printLogsb.append("打印申请-借货打印异常：");
                    //printLogsb.append("申请打印的借货还货编号："+idarrs);
                    //printLogsb.append(" 。");
                    printLogsb.append("借货编号： " + canprintIds);
                }
                ...
                if ("2".equals(billtype)) {
                    printLogsb.append("打印预览申请-还货打印预览异常：");
                } else {
                    printLogsb.append("打印预览申请-借货打印预览异常：");
                }
            }
            printLogsb.append("Exception ：lendPrint()>>>>>>>>>>>>>>>>异常信息: " + ex.getMessage());
            addPrintLogInfo("PrintHandle-lend", printLogsb.toString(), null);
            if ("2".equals(billtype)) {
                logger.error("还货单打印及预览处理异常", ex);
            } else{
                logger.error("借货单打印及预览处理异常", ex);
            }
     ...
     * @date 2014-1-6
     */
    public void lendPrintCallBackHandle(String printcallback,String billtype) throws Exception {
        String idarrs = request.getParameter("idarrs");
        if (null == idarrs) {
            idarrs = "";
        }
        ...
        if (null == printcallback || "".equals(printcallback)) {
            ...
            printLogsb.append(" 。");
            addPrintLogInfo("PrintCallBackHandle-lend", printLogsb.toString(), null);
            response.reset();
        ...
        if (null != id && !"".equals(id.trim())) {
            ...
            //printLogsb.append(" 。");
            if ("2".equals(billtype)) {
                printLogsb.append("还货编号： " + id);
            } else {
                printLogsb.append("借货编号： " + id);
            }
            try {
                Lend lend = lendService.showPureLend(id);
                if (null != lend) {
                    lend.setPrinttimes(1);
                    lendService.updateOrderPrinttimes(lend);
                ...
                if ("2".equals(billtype)) {
                    logger.error("还货单更新打印次数异常", ex);
                } else {
                    logger.error("借货单更新打印次数异常", ex);
                }
            }
        } else {
            printLogsb.append(" 。");
            if ("2".equals(billtype)) {
                printLogsb.append("回调参数中无还货编号信息");
            } else {
                printLogsb.append("回调参数中无借货编号信息");
            }
        }
        printLogsb.append(" 。");
		/*
		...
		*/
        //printLogsb.append(" 更新响应来源于： 借货还货打印。");
        if ("2".equals(billtype)) {
            printLogsb.append(" 打印数据来源(还货编号)：");
        } else {
            printLogsb.append(" 打印数据来源(借货编号)：");
        }
     ...
     * @date Nov 14, 2016
     */
    public boolean lendPrintReflectCallBackHandle(String printcallback) throws Exception {
        boolean isok = false;
        StringBuilder printLogsb = new StringBuilder();
        //printLogsb.append("更新打印次数：");
        if (null == printcallback || "".equals(printcallback)) {
        ...
        if (null != id && !"".equals(id.trim())) {
            ...
            try {
                ...
                if (null != lend) {
                    lend.setPrinttimes(1);
                    isok = lendService.updateOrderPrinttimes(lend);
                    printLogsb.append(" 更新打印次数成功");
                } else {
                    printLogsb.append(" 更新打印次数失败");
                }

 ...
 * @author zhanghonghui
 */
public class PlannedOrderPrintAction extends BaseFilesAction {
    //region 参数初始化
    private static final Logger logger = Logger.getLogger(PlannedOrderPrintAction.class);
     ...
     * @date 2015-01-14
     */
    public String plannedOrderPrintView() throws Exception {
        return plannedOrderPrintHandle(true);
    }
    ...
    public String plannedOrderPrint() throws Exception {
        ...
        if (null != printcallback && !"".equals(printcallback)) {
            plannedOrderPrintCallBackHandle(printcallback);
        } else {
            ...
            return plannedOrderPrintHandle(isview);
        }
        return null;
    }
     ...
     * @date 2015-01-14
     */
    private String plannedOrderPrintHandle(boolean isview) throws Exception {
        ...
        ajaxResultMap.put("printname", "采购计划单打印-" + CommonUtils.getDataNumberSeconds());
        ...
        try {
            ...
            if(isview){
                ...
                ajaxResultMap.put("printname", "采购计划单打印任务-" + printJob.getId());
            }
            String printlimit = getPrintLimitInfo();
            Map map = new HashMap();
            map.put("idarrs", idarrs);
            map.put("showdetail", "1");
            List<PlannedOrder> list = plannedOrderService.showPlannedOrderListBy(map);
            if (list.size() == 0) {
                if ("ajaxhtml".equals(viewtype)) {
                    ajaxResultMap.put("flag", false);
                    ajaxResultMap.put("msg", "未能找到相关打印单据");
            ...
            for (PlannedOrder item : list) {
                ...
                List<PlannedOrderDetail> detailList = item.getPlannedOrderDetailList();
                ...
                if (null != detailList && detailList.size() > 0) {
                    ...
                    templetQueryMap.put("code", "purchase_plannedorder");
                    ...
                    //String reportModelFile = servletContext.getRealPath("/ireport/purchase_plannedorder/purchase_plannedorder.jasper");
                    ...
                    //parameters.put("P_PlannedOrder", orderInfo);
                    ...
                    /**打印模板参数用的单据信息**/
                    PlannedOrder orderInfo = (PlannedOrder) CommonUtils.deepCopy(item);
                    orderInfo.setPlannedOrderDetailList(null);
                    ...
                    if (null != jrprint) {
                        jrprint.setName("采购计划单-" + item.getId());
                        jrlist.add(jrprint);
                        ...
                        if (!isview) {
                            ...
                            if (StringUtils.isNotEmpty(printJob.getId())) {
                                ...
                                printJobCallHandle.setClassname("plannedOrderPrintAction");
                                printJobCallHandle.setMethodname("plannedOrderPrintReflectCallBackHandle");
                                ...
                                printJobCallHandle.setPrintordername("采购计划单打印");
                                printJobCallHandle.setSourceorderid(item.getId());
                                printJobCallHandle.setSourceordername("采购计划单打印");
            ...
            if (!isview) {
                ...
                //printLogsb.append("申请打印的采购计划单编号："+idarrs);
                //printLogsb.append(" 。");
                printLogsb.append(" 采购计划单编号： " + canprintIds);
                printLogsb.append(" 。");
                printLogsb.append(" 操作名称： 采购计划单打印。");
                addPrintLogInfo("PrintHandle-plannedOrder", printLogsb.toString(), null);
            }
            //输出打印报表
            if ("ajaxhtml".equals(viewtype)) {
            ...
            StringBuilder printLogsb = new StringBuilder();
            if (!isview) {
                printLogsb.append("打印申请-采购计划单打印异常：");
                //printLogsb.append("申请打印的采购计划单编号："+idarrs);
                //printLogsb.append(" 。");
                printLogsb.append("采购计划单编号： " + canprintIds);
                printLogsb.append(" 。");
            } else {
                printLogsb.append("打印预览申请-采购计划单打印预览异常：");
            }
            printLogsb.append("Exception ：plannedOrderPrint()>>>>>>>>>>>>>>>>异常信息： " + ex.getMessage());
            addPrintLogInfo("PrintHandle-plannedOrder", printLogsb.toString(), null);
            logger.error("采购计划单打印及预览处理异常", ex);
     ...
     * @date 2014-1-6
     */
    public void plannedOrderPrintCallBackHandle(String printcallback) throws Exception {
        String idarrs = request.getParameter("idarrs");
        if (null == idarrs) {
            idarrs = "";
        }
        ...
        if (null == printcallback || "".equals(printcallback)) {
            ...
            printLogsb.append(" 。");
            addPrintLogInfo("PrintCallBackHandle-plannedOrder", printLogsb.toString(), null);
            response.reset();
        ...
        if (null != id && !"".equals(id.trim())) {
            //printLogsb.append("申请打印的采购计划单编号："+idarrs);
            //printLogsb.append(" 。");
            printLogsb.append("采购计划单编号： " + id);
            try {
                PlannedOrder plannedOrder = plannedOrderService.showPurePlannedOrder(id);
                if (null != plannedOrder) {
                    plannedOrder.setPrinttimes(1);
                    plannedOrderService.updateOrderPrinttimes(plannedOrder);
                ...
                logger.error("采购计划单更新打印次数异常", ex);
            }
        } else {
            printLogsb.append(" 。");
            printLogsb.append("回调参数中无采购计划单编号信息");
        }
        printLogsb.append(" 。");
        /*
		...
		*/
        //printLogsb.append(" 更新响应来源于： 采购计划单打印。");
        printLogsb.append(" 打印数据来源(采购计划单编号)：");
     ...
     * @date 2014-1-6
     */
    public boolean plannedOrderPrintReflectCallBackHandle(String printcallback) throws Exception {
        boolean isok = false;
        StringBuilder printLogsb = new StringBuilder();
        //printLogsb.append("更新打印次数：");
        if (null == printcallback || "".equals(printcallback)) {

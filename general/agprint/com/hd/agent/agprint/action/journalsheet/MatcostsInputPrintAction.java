...
public class MatcostsInputPrintAction extends BaseFilesAction {
    //region 参数初始化
    private static final Logger logger = Logger.getLogger(MatcostsInputPrintAction.class);
     ...
     * @date 2017-01-20
     */
    public String matcostsInputPrintView() throws Exception {
        return matcostsInputPrintHandle(true);
    }
     ...
     * @date 2017-01-20
     */
    @UserOperateLog(key = "matcostsInput-Storage", type = 0)
    public String matcostsInputPrint() throws Exception {
        ...
        if (null != printcallback && !"".equals(printcallback)) {
            matcostsInputPrintCallBackHandle(printcallback);
        } else {
            return matcostsInputPrintHandle(!("1".equals(justprint) || "true".equals(justprint)));
        }
        return null;
    }
     ...
     * @date 2017-01-20
     */
    private String matcostsInputPrintHandle(boolean isview) throws Exception {
        ...
        ajaxResultMap.put("printname", "代垫录入打印-" + CommonUtils.getDataNumberSeconds());
        ...
        try {
            ...
            if(isview){
                ...
                ajaxResultMap.put("printname", "代垫录入打印任务-" + printJob.getId());
            }
            String printlimit = getPrintLimitInfo();
            Map map = new HashMap();
            map.put("idarrs", idarrs);
            List<MatcostsInput> list = journalSheetService.getMatcostsInputListBy(map);
            if (list.size() == 0) {
                if ("ajaxhtml".equals(viewtype)) {
                    ajaxResultMap.put("flag", false);
                    ajaxResultMap.put("msg", "未能找到相关打印单据");
            ...
            templetQueryMap.put("code", "journalsheet_matcostsinput");
            ...
            //String reportModelFile = servletContext.getRealPath("/ireport/journalsheet_matreimburse/journalsheet_matcostsinput.jasper");
            ...
            for (MatcostsInput matcostsInput : list) {
                List<MatcostsInput> list1 = null;
                list1 = new ArrayList<MatcostsInput>();
                list1.add(matcostsInput);

                JasperPrint jrprint = JasperFillManager.fillReport(jreport, parameters, new JRBeanCollectionDataSource(list1));
                if (null != jrprint) {
                    jrprint.setName("代垫录入-" + sdf.format(date));
                    jrlist.add(jrprint);
                    ...
                    if (!isview) {
                        ...
                        if (StringUtils.isNotEmpty(printJob.getId())) {
                            ...
                            printJobCallHandle.setClassname("matcostsInputPrintAction");
                            printJobCallHandle.setMethodname("matcostsInputPrintReflectCallBackHandle");
                            ...
                            printJobCallHandle.setPrintordername("代垫录入打印");
                            printJobCallHandle.setSourceorderid(idarrs);
                            printJobCallHandle.setSourceordername("代垫录入打印");
            ...
            if (!isview) {
                ...
                //printLogsb.append(" 。");
                printLogsb.append(" 代垫录入编号： " + canprintIds);
                printLogsb.append(" 。");
                printLogsb.append(" 操作名称： 代垫录入打印。");
                addPrintLogInfo("PrintHandle-matcostsinput", printLogsb.toString(), null);
            }
            //输出打印报表
            if ("ajaxhtml".equals(viewtype)) {
            ...
            StringBuilder printLogsb = new StringBuilder();
            if (!isview) {
                printLogsb.append("打印申请-代垫录入打印异常：");
                //printLogsb.append("申请打印的代垫收回编号："+idarrs);
                //printLogsb.append(" 。");
                printLogsb.append("代垫录入编号： " + canprintIds);
                printLogsb.append(" 。");
            } else {
                printLogsb.append("打印预览申请-代垫录入打印预览异常：");
            }
            printLogsb.append("Exception ：matcostsinputPrint()>>>>>>>>>>>>>>>>代垫录入打印异常 " + ex.getMessage());
            addPrintLogInfo("PrintHandle-matcostsinput", printLogsb.toString(), null);
    ...
    public void matcostsInputPrintCallBackHandle(String printcallback) throws Exception {
        ...
        if (null == printcallback || "".equals(printcallback)) {
            ...
            printLogsb.append(" 。");
            addPrintLogInfo("PrintCallBackHandle-matcostsinput", printLogsb.toString(), null);
            response.reset();
        ...
        if (null != idArr && idArr.length > 0) {
            ...
            printLogsb.append(" 。");
            printLogsb.append("更新打印次数失败：回调参数中无代垫录入编号信息");
        }
        printLogsb.append(" 。");
        printLogsb.append(" 打印数据来源(代垫录入编号)：");
     ...
     * @date Jan 12, 2016
     */
    public boolean matcostsInputPrintReflectCallBackHandle(String printcallback) throws Exception {
        boolean isok = false;
        StringBuilder printLogsb = new StringBuilder();
        //printLogsb.append("更新打印次数：");
        if (null == printcallback || "".equals(printcallback)) {

...
public class DeptDailyCostPrintAction extends BaseFilesAction {
    //region 参数初始化
    private static final Logger logger = Logger.getLogger(DeptDailyCostPrintAction.class);
     ...
     * @date Jan 11, 2016
     */
    public String deptDailyCostPrintView() throws Exception {
        return deptDailyCostPrintHandle(true);
    }
    ...
    public String deptDailyCostPrint() throws Exception {
        ...
        if (null != printcallback && !"".equals(printcallback)) {
            ...
            deptDailyCostPrintCallBackHandle(printcallback);
            ...
            return deptDailyCostPrintHandle(view);
        }
        return null;
    }
     ...
     * @date Jan 11, 2016
     */
    private String deptDailyCostPrintHandle(boolean isview) throws Exception {
        ...
        ajaxResultMap.put("printname", "部门日常费用单打印-" + CommonUtils.getDataNumberSeconds());
        ...
        try {
            ...
            if(isview){
                ...
                ajaxResultMap.put("printname", "部门日常费用单打印-" + printJob.getId());
            }
            String printlimit = getPrintLimitInfo();
            Map map = new HashMap();
            map.put("idarrs", idarrs);
            map.put("noDataSql", "1");
            List<DeptDailyCost> list = deptDailyCostService.showDeptDailyCostListBy(map);
            if (list.size() == 0) {
                if ("ajaxhtml".equals(viewtype)) {
                    ajaxResultMap.put("flag", false);
                    ajaxResultMap.put("msg", "未能找到相关打印单据");
            ...
            templetQueryMap.put("code", "journalsheet_deptdailycost");
            ...
            //String reportModelFile = servletContext.getRealPath("/ireport/journalsheet_deptdailycost/journalsheet_deptdailycost.jasper");
            ...
            if (null != jrprint) {
                jrprint.setName("部门日常费用单-" + sdf.format(date));
                jrlist.add(jrprint);
                ...
                if (!isview) {
                    ...
                    if (StringUtils.isNotEmpty(printJob.getId())) {
                        ...
                        printJobCallHandle.setClassname("deptDailyCostPrintAction");
                        printJobCallHandle.setMethodname("deptDailyCostPrintReflectCallBackHandle");
                        ...
                        printJobCallHandle.setPrintordername("部门日常费用单打印");
                        printJobCallHandle.setSourceorderid(idarrs);
                        printJobCallHandle.setSourceordername("部门日常费用单打印");
            ...
            if (!isview) {
                ...
                //printLogsb.append("申请打印的部门日常费用单编号："+idarrs);
                //printLogsb.append(" 。");
                printLogsb.append(" 部门日常费用单编号： " + canprintIds);
                printLogsb.append(" 。");
                printLogsb.append(" 操作名称： 部门日常费用单打印。");
            ...
            StringBuilder printLogsb = new StringBuilder();
            if (!isview) {
                printLogsb.append("打印申请-部门日常费用单打印异常：");
                //printLogsb.append("申请打印的部门日常费用单编号："+idarrs);
                //printLogsb.append(" 。");
                printLogsb.append("部门日常费用单编号： " + canprintIds);
                printLogsb.append(" 。");
            } else {
                printLogsb.append("打印预览申请-部门日常费用单打印预览异常：");
            }
            printLogsb.append("Exception ：deptdailycostPrint()>>>>>>>>>>>>>>>>异常信息： " + ex.getMessage());
            addPrintLogInfo("PrintHandle-deptdailycost", printLogsb.toString(), null);
            logger.error("部门日常费用单打印及预览处理异常", ex);
     ...
     * @date Jan 12, 2016
     */
    public void deptDailyCostPrintCallBackHandle(String printcallback) throws Exception {
        ...
        if (null != idArr && idArr.length > 0) {
            ...
            for (String id : idArr) {
                printLogsb.append("部门日常费用单编号： " + id);
                try {
                    DeptDailyCost deptDailyCost = deptDailyCostService.getDeptDailyCostPureInfo(id);
                    if (null != deptDailyCost) {
                        deptDailyCostService.updateDeptDailyCostPrinttimes(deptDailyCost);
                    ...
                    logger.error("部门日常费用单更新打印次数异常", ex);
                }
            }
        } else {
            printLogsb.append(" 。");
            printLogsb.append("更新打印次数失败：回调参数中无部门日常费用单编号信息");
        }
        printLogsb.append(" 。");
        printLogsb.append(" 打印数据来源(部门日常费用单编号)：");
     ...
     * @date Nov 10, 2016
     */
    public boolean deptDailyCostPrintReflectCallBackHandle(String printcallback) throws Exception {
        boolean isok = false;
        StringBuilder printLogsb = new StringBuilder();
        //printLogsb.append("更新打印次数：");
        if (null == printcallback || "".equals(printcallback)) {

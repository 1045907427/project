 ...
 * @author zhanghonghui
 */
public class RejectBillPrintAction extends BaseFilesAction {
    //region 参数初始化
    private static final Logger logger = Logger.getLogger(RejectBillPrintAction.class);
     ...
     * @date 2013-11-12
     */
    public String rejectBillPrintView() throws Exception {
        return rejectBillPrintHandle(true);
    }
    ...
    public String rejectBillPrint() throws Exception {
        ...
        if (null != printcallback && !"".equals(printcallback)) {
            rejectBillPrintCallBackHandle(printcallback);
        } else {
            ...
            return rejectBillPrintHandle(isview);
        }
        return null;
    }
     ...
     * @date 2013-11-12
     */
    private String rejectBillPrintHandle(boolean isview) throws Exception {
        ...
        ajaxResultMap.put("printname", "销售退货通知单打印-" + CommonUtils.getDataNumberSeconds());
        ...
        if (null == billtype || !"phone".equals(billtype.trim())) {
            billtype = "bill";
        } else {
        ...
        try {
            ...
            if(isview){
                ...
                ajaxResultMap.put("printname", "销售退货通知单打印任务-" + printJob.getId());
            }
            Map map = new HashMap();
            ...
            List<RejectBill> list = salesRejectBillService.getRejectBillListBy(map);
            ...
            if ("phone".equals(billtype)) {
                templetQueryMap.put("code", "sales_rejectbill_phone");
                //reportModelFile=servletContext.getRealPath("/ireport/sales_rejectbill/sales_rejectbill_phone.jasper");
            } else {
                templetQueryMap.put("code", "sales_rejectbill");
            ...
            for (RejectBill item : list) {
                ...
                List<RejectBillDetail> detailList = item.getBillDetailList();
                ...
                parameters.put("P_Storage", item.getStoragename());    //仓库编号
                parameters.put("P_Businessdate", item.getBusinessdate()); //业务日期
                parameters.put("P_OrderId", item.getId()); //单据号
                parameters.put("P_SaleArea", customerInfo.getSalesareaname());    //销售片区


                parameters.put("P_CustomerAddr", customerInfo.getAddress());    //客户地址
                parameters.put("P_Contact", customerInfo.getContact());    //通用版，使用contact
                parameters.put("P_ContactTel", customerInfo.getMobile());

                parameters.put("P_RejectBillType", item.getBilltype());
                ...
                parameters.put("P_BillAddUser", item.getAddusername());    //制单人
                parameters.put("P_BillAdduserTel", addPersonnel.getTelphone());

                /**打印模板参数用的单据信息**/
                RejectBill orderInfo = (RejectBill) CommonUtils.deepCopy(item);
                ...
                if (null != jrprint) {
                    jrprint.setName("销售退货通知单-" + item.getId());
                    jrlist.add(jrprint);
                    ...
                    if (!isview) {
                        ...
                        if (StringUtils.isNotEmpty(printJob.getId())) {
                            ...
                            printJobCallHandle.setClassname("rejectBillPrintAction");
                            printJobCallHandle.setMethodname("rejectBillPrintReflectCallBackHandle");
                            ...
                            printJobCallHandle.setPrintordername("销售退货通知单打印");
                            printJobCallHandle.setSourceorderid(item.getId());
                            printJobCallHandle.setSourceordername("销售退货通知单打印");
            ...
            if (!isview) {
                ...
                //printLogsb.append("申请打印的销售退货通知单编号："+idarrs);
                //printLogsb.append(" 。");
                printLogsb.append("销售退货通知单编号： " + canprintIds);
                printLogsb.append(" 。");
                printLogsb.append(" 操作名称： 销售退货通知单打印。");
            ...
            StringBuilder printLogsb = new StringBuilder();
            if (!isview) {
                printLogsb.append("打印申请-销售退货通知单打印异常：");
                //printLogsb.append("申请打印的销售退货通知单编号："+idarrs);
                //printLogsb.append(" 。");
                printLogsb.append("销售退货通知单编号： " + canprintIds);
                printLogsb.append(" 。");
            } else {
                printLogsb.append("打印预览申请-销售退货通知单打印预览异常：");
            }
            printLogsb.append("Exception ：rejectBillPrintHandle()>>>>>>>>>>>>>>>>异常信息： " + ex.getMessage());
            addPrintLogInfo("PrintHandle-rejectBill", printLogsb.toString(), null);
            logger.error("销售退货通知单打印及预览处理异常", ex);
     ...
     * @date 2014-1-6
     */
    public void rejectBillPrintCallBackHandle(String printcallback) throws Exception {
        String idarrs = request.getParameter("idarrs");
        if (null == idarrs) {
            idarrs = "";
        }
        ...
        if (null == printcallback || "".equals(printcallback)) {
            ...
            printLogsb.append(" 。");
            addPrintLogInfo("PrintCallHandle-rejectBill", printLogsb.toString(), null);
            response.reset();
        ...
        if (null != id && !"".equals(id.trim())) {
            //printLogsb.append("申请打印的销售退货通知单编号："+idarrs);
            //printLogsb.append(" 。");
            printLogsb.append("销售退货通知单编号： " + id);
            try {
                RejectBill rejectBill = salesRejectBillService.getPureRejectBill(id);
                if (null != rejectBill) {
                    rejectBill.setPrinttimes(1);
            ...
            printLogsb.append(" 。");
            printLogsb.append("更新打印次数失败：回调参数中无销售退货通知单编号信息");
        }
        printLogsb.append(" 。");
        printLogsb.append(" 打印数据来源(销售退货通知单编号)：");
     ...
     * @date 2014-1-6
     */
    public boolean rejectBillPrintReflectCallBackHandle(String printcallback) throws Exception {
        boolean isok = false;
        StringBuilder printLogsb = new StringBuilder();
        //printLogsb.append("更新打印次数：");
        if (null == printcallback || "".equals(printcallback)) {

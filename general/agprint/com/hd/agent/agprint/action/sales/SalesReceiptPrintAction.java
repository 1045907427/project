 ...
 * @author master
 */
public class SalesReceiptPrintAction extends BaseFilesAction {
    //region 参数初始化
    private static final Logger logger = Logger.getLogger(SalesReceiptPrintAction.class);
    ...
    private String salesOrderPrintHandle(boolean isview) throws Exception {
        ...
        ajaxResultMap.put("printname", "在销售订单中-打印-" + CommonUtils.getDataNumberSeconds());
        ...
        try {
            ...
            List<Receipt> list = salesReceiptService.getReceiptListBy(paramMap);
            if (list.size() == 0) {
                if ("ajaxhtml".equals(viewtype)) {
                    ajaxResultMap.put("flag", false);
            ...
            for (Receipt item : list) {
                ...
                List<ReceiptDetail> detailList = item.getReceiptDetailList();
                ...
                for (ReceiptDetail orderDetail : detailList) {
                ...
                /**打印模板参数用的单据信息**/
                Receipt orderInfo = (Receipt) CommonUtils.deepCopy(item);
                orderInfo.setReceiptDetailList(null);
                ...
                if (null != jrprint) {
                    if (!isview) {
                        ...
                        callbackparamMap.put("orderPrintCount", orderPrintCount);    //当前销售回单打印次数
            ...
            if (!isview) {
                ...
                //printLogsb.append("申请打印销售回单编号："+saleidarrs);
                //printLogsb.append(" 。");
                printLogsb.append("销售回单编号： " + orderCanprintIds);
                printLogsb.append(" 。");
                printLogsb.append(" 操作名称：在销售回单中-打印");
                printLogsb.append(" 。");
            ...
            StringBuilder printLogsb = new StringBuilder();
            if (!isview) {
                printLogsb.append("打印申请-在销售回单中-打印异常：");
                //printLogsb.append("申请打印销售回单编号："+saleidarrs);
                //printLogsb.append(" 。");
                printLogsb.append("销售回单编号： " + orderCanprintIds);
                printLogsb.append(" 。");
            } else {
                printLogsb.append("打印预览申请-在销售回单中-打印预览异常：");
                printLogsb.append("预览的销售回单编号：" + saleidarrs);
                printLogsb.append(" 。");
            }
            printLogsb.append("Exception ：salesOrderPrintHandle()>>>>>>>>>>>>>>>>异常信息： " + ex.getMessage());
            addPrintLogInfo("PrintHandle-salesOrder", printLogsb.toString(), null);

            logger.error("在销售回单中-打印及预览处理异常", ex);
    ...
    public void salesOrderCallBackHandle(String printcallback) throws Exception {
        ...
        if (null != id && !"".equals(id.trim())) {
            ...
            try {
                Receipt order = salesReceiptService.getReceipt(id);
                if (null != order) {
                    order.setPrinttimes(1);
                    salesReceiptService.updateOrderPrinttimes(order);
                ...
                logger.error("在销售回单中-更新打印次数异常", ex);
            }
        } else {
            printLogsb.append(" 。");
        ...
        printLogsb.append(" 打印数据来源(销售回单编号)：");
